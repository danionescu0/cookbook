...
- [x] Per-user accounts: self-service signup (`POST /auth/signup`, username/email/password behind
      ...
      in until it clicks the link from a verification email, sent by the worker over SMTP. A
...
│   │   │                            #   recipes" with a share/unshare toggle — renders ImportManager
│   │   │                            #   from backoffice/ too, since /imports is scoped per-caller),
...
│   │   │                            #   LoginForm — all gated behind AuthContext. App.tsx's
│   │   │                            #   BackofficeLayout renders LoginForm when unauthenticated,
│   │   │                            #   else a Recipes/Settings sub-nav + <Outlet /> across nested
│   │   │                            #   /backoffice routes
...
│   │   │                           #   /verify-email, /login), users.py (GET /users/me — incl.
│   │   │                           #   imported_recipes_count, PATCH /users/me/password),
│   │   │                           #   categories.py, recipes.py (CRUD + /approve + /share +
│   │   │                           #   /reparse + /reparse-all-imported, owner/visibility-scoped,
│   │   │                           #   plus /users/me/favorites + /users/me/submissions on a second
...
│   │                                #   users.imported_recipes_count 0025
│   ├── requirements.txt
...
└── worker/                         # RabbitMQ consumer: fetch -> Claude -> download images -> insert
    ...
    │   │                           #   the admin-configured host/port/credentials
    ...
    │   │                           #   recipe.estimated_servings; skips a line matching
    ...
    │   │                           #   estimated_servings; translate_recipe — third tool-use call,
...
2. The account is created with `is_verified=False`, and an `email_jobs` row is queued for the
   worker, which sends a verification email over the admin-configured SMTP settings, linking back
   to `public_site_url + /verify-email?token=...`.
...
### 1. Manual authoring
Any signed-in user creates/edits/deletes their own recipes from their account page (full CRUD, no
AI involved); an admin can additionally edit/delete anyone's from the back office. A newly-created
recipe is private to its owner unless they check "share with the community," in which case a
non-admin's goes to `unapproved` for the existing moderation step (workflow #4) and an admin's goes
live immediately.
...
6. Worker inserts the recipe into PostgreSQL already `status = approved`, `owner_user_id` set to
   whoever created the import job, and `is_shared = False` (imports are never shareable — see
   [Design Decisions](#design-decisions)), with one `recipe_translations` row per language, and
   `added_at`/`approved_at` timestamps. In the same transaction, the worker increments the owner's
   lifetime `imported_recipes_count` by one — this is what workflow step 1's limit check reads.
   Right after that commit, the worker automatically queues a nutrition job for the new recipe —
...
### 5. Editing a recipe
1. Any signed-in owner (or an admin, for anyone's recipe) edits a recipe's title/description/
   ingredients/steps/tips/images from the back office (`RecipeManager`) or their account page
   (`RecipeSubmitForm`'s edit path). `PUT /recipes/{id}` applies the change to whichever language
   was being edited.
...
1. An admin clicks **Reparse** on an imported recipe in `RecipeManager` (or **Reparse all imported
   recipes** to queue every recipe with a `source_url`, e.g. after a prompt/extraction-quality fix
   like the sub-group convention). `POST /recipes/{id}/reparse` (or `/reparse-all-imported`) 400s if
   the recipe has no `source_url` — there's nothing to re-scrape for a hand-authored recipe — and
   creates one `recipe_reparse_jobs` row per recipe, published to its own queue immediately (no
   pending/approve gate — the admin already imported this exact recipe once).
...
- **users** — id, username (unique), email (unique, nullable — nullable only for the
  ...
  [Design Decisions](#design-decisions)), is_verified, imported_recipes_count (lifetime count of
  *successful* imports, default 0, incremented once by the worker per import and never
  ...
  [Design Decisions](#design-decisions), "Lifetime import limit"), created_at
...
- **recipes** — id, category_id, images, source_url (nullable, set when imported), owner_user_id
  (FK → users.id, not nullable — every recipe belongs to exactly one user), is_shared (bool,
  default false — only ever meaningful when source_url is null; imports never set it), status
  ...
  content lives in **recipe_translations**. Visibility (see
  [Design Decisions](#design-decisions)): an admin sees everything; the owner always sees their own
  (any status); anyone else sees it only if `status=approved AND is_shared=true`
...
- **import_jobs** — id, category_id (recipe lands here on success), created_by_user_id (FK →
  ...
  For an admin, `pending` means created but not yet approved/published to RabbitMQ; for everyone
  else it's queued immediately (see [Core Workflows](#core-workflows), workflow #2)
...
- **Manual hack: promoting an imported recipe to public** — there is deliberately no UI or API
  path for this (see "Per-user accounts and recipe privacy" above: imports can never be shared,
  permanently, by design), but the need still comes up in practice — e.g. wanting one specific
  imported recipe to be publicly visible. The only way is to reclassify it as if it had been
  manually added, by clearing `source_url` (the column the app actually uses to decide "is this an
  import") and setting the sharing/approval flags directly:
  ```bash
...
- **Securely accessing the prod database from a local GUI client** — in dev, `db` already
  publishes `5432` straight to the host (`docker-compose.override.yml`), so any local Postgres
  client (DBeaver, TablePlus, Beekeeper Studio, ...) can just connect to `localhost:5432`. Prod
