...
**Live at [easy2cook.app](https://easy2cook.app)** — the site itself links back here from its
footer on every page (AGPL-3.0, see [License](#license)).
...
**Accounts** — self-service signup behind a Cloudflare Turnstile CAPTCHA. Login is by email now, not
a separate username — see [Design Decisions](#design-decisions) ("Removing usernames"):

<img src="docs/screenshots/signup-turnstile.jpg" alt="Signup form with email/password fields and a Cloudflare Turnstile CAPTCHA" width="800">

The same login form serves regular users and admins — there's no separate back-office login. A
"Forgot your password?" link starts the self-service reset flow:

<img src="docs/screenshots/backoffice-login.jpg" alt="Login form with email/password fields, a link to sign up, a forgot-password link, and a note that admin access uses the same login" width="800">
...
- [x] Back office import request flow (`ImportManager`): submitting a URL creates a
      `pending` `import_jobs` row that is **not** sent to RabbitMQ yet; **Approve** is what
      publishes it to the queue, and the UI calls it itself the instant the row is created — so in
      practice submitting a URL just starts the import, with no second click. The button stays in
      the UI as a manual fallback, and doubles as retry for a `failed` job. The list polls every 3s
      while any job is `queued`/`fetching`/`processing`. A successful import now publishes
...
- [x] Per-user accounts: self-service signup (`POST /auth/signup`, email/password behind
      ...
      signup, independent of `default_language`/browsing-language changes afterward) with a
      one-paragraph "what this site is" blurb, not just a bare link. A
      ...
      serves regular users and admins, there's no separate back-office login. There is no username —
      email is the only account identifier (see [Design Decisions](#design-decisions), "Removing
      usernames"). `JWT_SECRET`/`JWT_EXPIRES_MINUTES` stay `.env`-only.
- [x] Self-service password recovery: `POST /auth/forgot-password` (CAPTCHA-gated) always returns
      the same generic response regardless of whether the email is registered — only a real match
      actually gets a `password_reset_tokens` row and an email — and `POST /auth/reset-password`
      consumes a one-hour, single-use token. A "Forgot your password?" link sits under the login
      form, and the signup form itself surfaces the same link inline when it rejects an
      already-registered email, instead of leaving the visitor at a dead end. See
      [Design Decisions](#design-decisions).
- [x] "Sign in with Google": one Google Identity Services button, shown on both the login and
      signup pages, that transparently logs in, links, or creates an account depending on what
      Google's proven email matches (`POST /auth/google`) — see
      [Core Workflows](#core-workflows) #0 and [Design Decisions](#design-decisions). No client
      secret is stored anywhere; the ID token's signature is verified against Google's own public
      keys. `app_settings.google_client_id` (admin-editable, not secret) gates it — the button
      renders nothing until it's configured.
...
- [x] Lifetime import limit per user (`app_settings.max_imports_per_user`, admin-configurable,
      ...
      [Design Decisions](#design-decisions).
- [x] Bookmark-file import: a separate "Import from bookmarks" panel on the Import page, below
      the single-URL form — upload a browser-exported bookmarks HTML file, pick which of the
      links found in it to import from a checklist, and each selected one becomes a normal
      `import_jobs` row (`type=bookmark`) that goes through the exact same fetch → extract →
      categorize pipeline, and the same "in progress" list, as any other import. Counts against
      the same lifetime `imported_recipes_count` quota as a single-URL import, so selection is
      capped at however many the user has left (admins unlimited); a link already imported or
      already queued is shown disabled with a badge rather than costing a quota slot. A "How do I
      export my bookmarks?" link opens a scrollable overlay with Chrome/Firefox/Safari/Internet
      Explorer instructions. See [Core Workflows](#core-workflows) and
      [Design Decisions](#design-decisions), "Bookmark import".
...
- [x] Superadmin Users page (`UsersManager`, `/backoffice/users`): a table of every account —
      email, confirmed/unconfirmed, joined date, last login, and lifetime
      ...
      explicit user id for an admin caller (previously only `"me"`), 403ing a non-admin and 404ing
      an unknown id. See [Design Decisions](#design-decisions).
...
- [x] Admin review tracking for failed imports, and the Import page's review panel scoped to the
...
- [x] AI-suggested import category, replacing manual pre-selection: the category `<select>` is
      gone from the "Import from URL" form — Claude picks the best-matching category from the
      app's real, current list as part of the same extraction call, and the worker resolves it to
      a `category_id` (falling back to the first category on no match, never failing the import
      over it). Shown as a small "AI-selected category: {name}" label on each card in the Account
      page's "Recently imported" panel; overridable any time via the recipe's own Edit page, which
      already has a category `<select>`. See [Design Decisions](#design-decisions),
      "AI-suggested import category".
- [x] Public **Contact** page (`/contact`), reachable both logged in and logged out: name
      (required), message (required, min 50 characters), and email/phone where at least one of
      the two is required (both may be given). Submissions queue on a new `contact_messages`
      table/`contact_message_jobs` queue — same async job pattern as every other worker task —
      and the worker emails the submission to the admin-configured
      `app_settings.contact_recipient_email` (Settings page). An anonymous submitter must solve a
      Turnstile CAPTCHA (same widget as signup); a logged-in one skips it entirely. See
      [Design Decisions](#design-decisions), "Contact page".
- [x] Local backups (`scripts/backup.sh`, cron-driven): Postgres + the `images_data` volume,
      dated and compressed under `./backups/`, pruned to keep today's, yesterday's, and one
      oldest-survivor each from a ~4-day and a ~8-day window (at most 4 backups of each kind).
      Getting a copy off the VPS is left to an external tool the operator already has, not baked
      into this repo. See [Deployment](#deployment) for setup + restore, and
      [Design Decisions](#design-decisions), "Local backup strategy".
- [x] DeepSeek as an alternate AI provider: an admin-editable `app_settings.preferred_ai_provider`
      (Settings page) switches every Claude call site (extraction, ingredient parsing,
      translation) to DeepSeek instead, reached through DeepSeek's own Anthropic-API-compatible
      endpoint — same `anthropic` SDK, same forced-tool-use request shape, just a different
      `base_url`/`api_key`/model, so no separate provider SDK or code path. See
      [Design Decisions](#design-decisions), "DeepSeek as an alternate AI provider".
- [x] A dedicated **Import** page (`/import`), separate from **Account** (`/account`): manual
      add/URL/bookmark import and "My recipes" moved off the account page onto their own,
      reachable via a header nav link and an "Add or import recipes" button on the recipes page.
      Favorites stopped being an Account section and became a **Favorites filter** on the recipes
      page instead — a separately-colored pill after the category pills, combinable with a
      category (not mutually exclusive with one), collapsing the usual My recipes/From the
      community split into one flat list while active. See
      [Design Decisions](#design-decisions), "Splitting Import out of the account page".
- [x] Recipe search box on the front-office recipes page, the backoffice `RecipeManager`, and the
      Import page's "My recipes" list: 3+ characters, 1.5s debounce, title-only, AND-substring
      multi-word matching, diacritic-insensitive in both directions ("briose" matches "Brioșe" and
      vice versa), composable with whatever other filter is already active. Server-side (new
      `search` param on `GET /recipes`, backed by a `pg_trgm` + `unaccent` GIN index, migration
      `0036`) on the first two; client-side on the Import page, whose list was already unpaginated.
      See [Design Decisions](#design-decisions), "Recipe search".
- [x] Person-to-person recipe sharing (`recipe_shares`, migration `0037`): a "Send to a friend"
      action on any owned recipe generates an open, multi-use, 24-hour link (`/share/:token`,
      `ShareRecipeDialog.tsx`). Opening it while logged out shows a teaser (thumbnail, title, a
      160-char-truncated description) plus Google/manual sign-up-or-log-in CTAs, without exposing
      the sender's email; once authenticated it shows the full recipe (via the same
      `RecipeDetailView` every other detail page uses) with a "Copy to my recipes" button that
      duplicates the recipe — translations, physically-copied image files, and nutrition/ingredient
      links all included — under the recipient's own account (`Recipe.shared_from_recipe_id` tracks
      provenance); that button is withheld (replaced with "This is your own recipe.") when the
      authenticated viewer already owns the recipe, e.g. testing their own link or re-opening it
      from their own sent message. A `cookbook-pending-share` localStorage key survives the multi-page manual-
      signup chain (signup → email verify → login) so the visitor lands back on the share page
      once authenticated, not on the home page. The "Send to a friend" entry point (a small
      share-icon pictogram, reused across all three) lives on every recipe card in
      `RecipeBrowser`/`RecipeGrid`, and next to the title on both `RecipeDetail` and
      `PublicRecipeDetail`. `ShareRecipeDialog` itself branches on whether the recipe is already
      public (shared with the community *and* approved, the same condition
      `routers/recipes.py`'s `_is_public` checks): a public recipe already has a permanent,
      no-account-needed URL, so the dialog just shows/copies that instead of generating a
      `recipe_shares` row — no expiry, no sign-up wall, since the recipe is already visible to
      anyone. See [Design Decisions](#design-decisions), "Recipe sharing", for why the link is
      open/multi-use, why images are duplicated rather than referenced, the public/private
      branching, and a StrictMode double-effect bug this surfaced in the redirect itself.
...
- **User accounts** — self-service signup behind a CAPTCHA, gated on email verification, plus a
  ...
  Accounts are identified by email only — there's no separate username (see
  [Design Decisions](#design-decisions), "Removing usernames").
- **Self-service password recovery** — a "Forgot your password?" link on the login page, and the
  same link surfaced inline on the signup form when it rejects an email that's already registered,
  so a returning visitor who mistypes their password never dead-ends. See
  [Design Decisions](#design-decisions).
- **Sign in with Google** — one button on both the login and signup pages, driven by Google
  Identity Services. Handles login, first-time account creation (behind the same Terms overlay
  manual signup uses), and linking to an existing password account by email, all through a single
  `POST /auth/google` endpoint. No client secret involved — only a Google Client ID (not secret,
  admin-editable) — since the ID token's signature is verified against Google's own public keys.
...
- **Recipe privacy, with opt-in community sharing** — recipes are private to whoever owns them by
  ...
  non-admin's recipe.
- **Send a recipe to a friend** — a share-icon button on every recipe card and detail page. For a
  private recipe (distinct from community sharing above), it's a 24-hour link a person can open,
  sign up or log in behind, and copy into their own account as a fully independent, editable
  recipe — which they can then send onward the same way. For a recipe that's already public, it's
  just that recipe's own permanent URL — no account needed, never expires. See
  [Design Decisions](#design-decisions), "Recipe sharing".
- **Back office** — email/password-protected admin area with full CRUD for categories and
...
- **URL and Instagram scraping, open to every user** — submit a URL (including an Instagram
  post/reel link) from your own account, no category needed up front — Claude suggests one from
  extraction. It starts fetching right away either way: a non-admin's import queues immediately for
  their own private use, and an admin's — which the API still lands as a `pending` row before
  anything is fetched — gets auto-approved by the page itself the moment it's created, so there's no
  extra click. See [Design Decisions](#design-decisions), "Approval gate moved to the import
  request, not the imported result".
- **Bookmark-file import** — upload a browser-exported bookmarks HTML file and pick which of the
  links found in it to import from a checklist, capped at however many imports you have left under
  your account's lifetime limit; each selected link then runs through the exact same
  fetch/Claude(or DeepSeek)/image pipeline as a single-URL import, and shows up in the same
  "in progress" list.
...
- **Review gate on the request, not the result** — since an admin explicitly approves every URL
  ...
  unpublished/republished via the API today. The frontend now makes that approval itself (see
  [Design Decisions](#design-decisions), "Approval gate moved to the import request, not the
  imported result") the moment the admin submits the URL — the *decision* still happens (typing/
  pasting this exact URL and clicking Add *is* the approval), it's just no longer a second click.
...
- **Support link** — a small, low-key note at the bottom of the logged-out landing page
  (`frontoffice/LandingPage.tsx`) links to a Buy Me a Coffee page, with a line explaining that it
  covers the VPS, domain, and AI subscription costs. Deliberately the quietest element on the page
  (muted color, small type, last thing before the footer) so it's discoverable without competing
  with the signup/browse CTAs.
...
- **Superadmin Users page** — a table of every account (email, confirmed status, joined
...
- **Contact page** — a public `/contact` form (name, email-or-phone, a min-50-character message)
  reachable whether signed in or not; a signed-out visitor solves a CAPTCHA, a signed-in one
  doesn't. Submissions are emailed to the admin-configured recipient via the same async
  queue-and-worker pattern as every other background task, not sent synchronously from the API.
- **Recipe search** — a search box on the front-office recipe browser, the back office recipe
  manager, and the Import page's "My recipes" list, all filtering by title as you type: it waits
  1.5s after typing stops, requires 3+ characters, treats a multi-word query as "every word must
  appear somewhere in the title" (any order), and ignores diacritics in both directions ("briose"
  matches "Brioșe" and vice versa). Composes with whatever other filter is already active
  (category, Favorites). The front office and back office search server-side, via a Postgres
  trigram + `unaccent` index; the Import page filters client-side, since that list was already
  loaded unpaginated. See [Design Decisions](#design-decisions).
...
| Scraper workers         | Python + Pika (RabbitMQ client), httpx (fetch), Playwright (headless-browser fetch for Instagram), BeautifulSoup4 with the `lxml` parser backend (strip boilerplate before it reaches Claude — `lxml` over stdlib `html.parser` for real-world malformed-HTML resilience, see Design Decisions), stdlib `urllib.robotparser`, Pillow (image resize/compress) | Consume import/reparse jobs, respect robots.txt + rate limits, fetch pages, clean HTML, download + process images |
| AI parsing/translation  | Claude API (Anthropic) or DeepSeek, both via the `anthropic` Python SDK — `claude-haiku-4-5`/`deepseek-v4-flash` for extraction/parsing, `claude-sonnet-5`/`deepseek-v4-pro` for translation | Bookmark HTML → candidate recipe links is a plain BeautifulSoup `<a href>` scan (`api/app/bookmark_parser.py`), not an AI call — Claude/DeepSeek only get involved once a link is actually selected for import, same as a single URL; raw (cleaned) page → cleaned recipe JSON, one translation per `SUPPORTED_LANGUAGES` entry, in a single tool-use call; a second tool-use call parses ingredient lines into structured food/quantity/unit for nutrition lookup; a third tool-use call re-translates a hand-edited recipe into every other language after a back-office edit or a reparse. Provider is an admin-editable setting (`app_settings.preferred_ai_provider`) — DeepSeek is reached through its own Anthropic-API-compatible endpoint, so no separate SDK or request-shape branch is needed — see Design Decisions ("Claude cost optimization", "DeepSeek as an alternate AI provider") |
...
| Account security         | `bcrypt` (password hashing), Cloudflare Turnstile (CAPTCHA on signup/resend/anonymous contact form), `email-validator` | Signup/resend/an anonymous contact submission are all CAPTCHA-gated; login is gated on email verification |
| Outbound email           | SMTP (stdlib `smtplib`), admin-configured host/port/credentials | Worker sends account-verification emails (`worker/app/email_handlers.py`) and contact-form notifications (`worker/app/contact_handlers.py`), each off its own queue |
| Message queue           | RabbitMQ                                                  | Decouples API from long-running scrape/import/nutrition/translation-sync/reparse/email/contact jobs; queues (`import_jobs`, `nutrition_jobs`, `ingredient_refresh_jobs`, `translation_sync_jobs`, `recipe_reparse_jobs`, `email_jobs`, `contact_message_jobs`) are all consumed by the same worker container |
...
Reference this before building any new frontend feature — reusing what's here keeps new UI
consistent with the rest of the app for free, instead of every screen re-deriving its own look and
feel. Most of this was established while building the recipe search box (see the "Recipe search"
Design Decision) and the confirm-dialog/pagination pieces before it; treat it as the baseline, not
a one-off.

- **Design tokens** (`src/index.css`, Tailwind v4 `@theme` block) — a warm/rustic palette:
  `cream`/`cream-card` (backgrounds), `ink` (text, used at full opacity for primary text and at
  `/50`–`/60` opacity for secondary/muted text — see state patterns below), `terracotta`/
  `terracotta-dark` (primary accent/CTA), `olive`/`olive-light` (borders, secondary surfaces), plus
  `font-serif` for headings. Reach for these token classes (`text-ink/60`, `border-olive/30`,
  `bg-terracotta`, etc.) rather than introducing new hex values or Tailwind's default palette.
- **Reusable primitives (`src/ui/`)** — check here before writing something new:
  - `buttonStyles.ts` — `primaryButton`, `secondaryButton`, `dangerButton` (outline, for
    triggering a destructive action), `dangerButtonSolid` (solid fill, reserved for the confirm
    step itself). All four already bake in `focus-visible:outline` and `disabled:` states.
  - `useConfirm.tsx` + `ConfirmDialog.tsx` — async, styled drop-in for `window.confirm(...)`
    (`if (!(await confirm(message))) return;`). Use this for every destructive action instead of
    the native browser dialog.
  - `sharing/ShareRecipeDialog.tsx` — not a shared primitive itself, but the reference for a
    *content-rich* modal (a list, not a single yes/no choice): reuses `ConfirmDialog`'s exact shell
    (portal via `createPortal`, backdrop/panel animation classes, `Escape`-to-close, body-scroll
    lock) with its own body. Copy this shape for the next dialog that needs more than confirm/cancel
    rather than stretching `ConfirmDialog` itself to fit.
  - `Pagination.tsx` — the shared page-number control; reuse for any new paginated list rather than
    hand-rolling prev/next buttons.
  - `useDebouncedValue.ts` / `useRecipeSearch.ts` — the generic debounce hook and its 1.5s/3-
    character-minimum specialization used by the recipe search boxes. Reuse `useRecipeSearch`
    verbatim for any new "filter as you type" input; reach for the lower-level
    `useDebouncedValue` only if the timing/threshold rules genuinely differ.
  - `searchMatch.ts` — the diacritic-insensitive, AND-substring, any-word-order title matcher used
    for client-side filtering (e.g. the Import page's already-loaded list). Reuse it rather than
    writing a new `.includes()` check for any similar client-side filter.
  - `nutritionIcons.tsx`, `hostnameOf.ts` — small presentational/formatting helpers; check before
    re-implementing similar formatting inline.
- **State pattern cookbook** — the same four states recur on every list/feed (`RecipeBrowser.tsx`,
  `RecipeManager.tsx`), rendered with these exact classes:
  - Loading (more): `<p className="text-sm text-ink/50">{t.section.loadingMore}</p>`
  - Error: `<p role="alert" className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p>`
  - Empty (nothing to show, no filter active): `<p className="text-ink/60">{t.section.empty}</p>`
  - No search/filter results (distinct from genuinely empty — don't conflate the two messages):
    `<p className="text-ink/60">{t.section.noSearchResults}</p>`, branched on whether a filter is
    active, e.g. `{search ? t.section.noSearchResults : t.section.empty}`
  - Infinite scroll: an `aria-hidden="true"` sentinel `<div>` observed via `IntersectionObserver`
    (see `useInfiniteRecipes.ts`), not a manual "load more" button.
- **Accessibility** — expected on every new interactive piece, not opt-in: `aria-labelledby` tying
  a `<section>` to its own heading `id`; `aria-label` on any input without a visible `<label>`
  (mirror the placeholder text, as the search boxes do); `focus-visible:outline` on every
  clickable/focusable element (already in `buttonStyles.ts` — replicate it on any one-off control);
  `role="alert"` on inline error text; `prefers-reduced-motion` respected for any new animation
  (see the `@media (prefers-reduced-motion: reduce)` block in `index.css`).
- **i18n** — every user-facing string goes through `t.section.key` from `useLanguage()`, never a
  hardcoded literal. A new string needs a key added to *both* `src/i18n/translations/en.ts` and
  `ro.ts` in the same change. The `Translation` type is inferred from `en.ts`, and `ro.ts` is
  declared as `const ro: Translation = {...}` — so a key missing from *either* file is a compile
  error (`tsc -b`/the frontend build catches it), not something that has to be checked by hand.
- **Testing** — every new interactive component/hook gets a Vitest + React Testing Library test in
  `src/tests/`, named after what it tests (`ComponentName.test.tsx`, `useSomething.test.ts`).
  Anything debounced follows `useRecipeSearch.test.ts`'s pattern: `vi.useFakeTimers({
  shouldAdvanceTime: true })`, `userEvent.setup({ advanceTimers: vi.advanceTimersByTime })`, and
  `await act(() => vi.advanceTimersByTimeAsync(ms))` around the wait.
- **Docs** — after a UI change, run the `update-readme` skill so this file (Status, Features, this
  section, Testing counts, Design Decisions) stays current instead of drifting from what's shipped.
...
├── .env.example
├── scripts/
│   └── backup.sh                   # cron-driven: pg_dump + tar of images_data into ./backups/,
│                                    #   kept-oldest-in-range retention (~4-day/~8-day slots,
│                                    #   see Design Decisions) — see Deployment
...
│   │   │                            #   community split, collapsing to one flat list while the
│   │   │                            #   Favorites filter is active — see Design Decisions,
│   │   │                            #   "Splitting Import out of the account page" — plus the "Add
│   │   │                            #   or import recipes" button linking to /import), RecipeDetail
│   │   │                            #   (/recipes/:id) + NutritionPanel (calories/protein/carbs/
│   │   │                            #   sugars/fat per serving, icon tiles), CategoryNav (category
│   │   │                            #   pills + a separately-colored, independently-toggled
│   │   │                            #   Favorites pill — combinable with a category, not part of
│   │   │                            #   the same single-select group), RecipeCard,
...
│   │   ├── account/                 # AccountPage (/account — just profile + change password now),
│   │   │                            #   ImportPage (/import — "Add a recipe" link, manual/bookmark
│   │   │                            #   import forms, and RecipeList of what the caller has already
│   │   │                            #   added, each with a share/unshare toggle, a delete button
│   │   │                            #   (DELETE /recipes/{id}, owner-or-admin, behind the same
│   │   │                            #   shared ConfirmDialog the backoffice delete uses), and a
│   │   │                            #   per-recipe category selector (PATCH /recipes/{id}/category,
│   │   │                            #   manual or imported alike) — renders ImportManager and
│   │   │                            #   BookmarkImportPanel from backoffice/ too, since /imports is
│   │   │                            #   scoped per-caller; see Design Decisions, "Splitting Import
│   │   │                            #   out of the account page" for why favorites became a
│   │   │                            #   RecipeBrowser filter instead of a section here),
│   │   │                            #   PendingImportsPanel (post-import review: an unreviewed
│   │   │                            #   import's card w/ Instagram play-button warning + an
│   │   │                            #   "AI-selected category: {name}" label + Edit link +
...
│   │   │                            #   admin-only actions, "back"/save-redirect goes to /import),
│   │   │                            #   RecipeSubmitForm ("Add a recipe", with a share-with-
│   │   │                            #   community checkbox, "back" link goes to /import)
│   │   ├── backoffice/              # CategoryManager, ImportManager (submit URL/Instagram link —
│   │   │                            #   no category picker any more, Claude suggests one after
│   │   │                            #   extraction, see PendingImportsPanel — approve-to-trigger,
│   │   │                            #   polls while active — reused inside AccountPage for
│   │   │                            #   non-admins; shows a non-admin's lifetime import usage and
│   │   │                            #   blurs+disables Add once the limit is hit; accepts an
│   │   │                            #   optional reloadTrigger prop so a job created elsewhere,
│   │   │                            #   e.g. BookmarkImportPanel's batch, shows up here too — see
│   │   │                            #   Design Decisions, "Bookmark import"),
│   │   │                            #   BookmarkImportPanel (separate "Import from bookmarks"
│   │   │                            #   section: load a bookmark HTML file, POST .../bookmark/parse
│   │   │                            #   returns candidate links, check the ones to import — capped
│   │   │                            #   at the caller's remaining quota, an already-imported link
│   │   │                            #   shown disabled with a badge — then POST .../bookmark
│   │   │                            #   creates one ImportJob per selection), plus
│   │   │                            #   HowToImportBookmarksDialog (a "How do I export my
│   │   │                            #   bookmarks?" link opens a scrollable overlay with per-browser
│   │   │                            #   steps, closed by a single OK button — same overlay shape as
│   │   │                            #   legal/TermsOverlay.tsx),
...
│   │   │                            #   + role flags in localStorage), SignupForm (email/password +
│   │   │                            #   Turnstile), ForgotPasswordPage (/forgot-password),
...
│   │   ├── contact/                 # ContactPage.tsx (/contact — reachable logged in or out;
│   │   │                            #   TurnstileWidget shown/required only when logged out, via
│   │   │                            #   useAuth's isAuthenticated)
...
│   │   │                           #   users.last_login_at — /forgot-password, /reset-password),
│   │   │                           #   users.py (GET /users/me — incl.
...
│   │   │                           #   accepts an explicit user id for an admin caller, not just
...
│   │   │                           #   their lifetime import limit; also POST /imports/bookmark/
│   │   │                           #   parse — reads an uploaded bookmark HTML file, returns
│   │   │                           #   candidate links + which are already imported + the caller's
│   │   │                           #   remaining quota, writes nothing — and POST /imports/bookmark
│   │   │                           #   — creates one ordinary ImportJob per selected URL, same
│   │   │                           #   admin/non-admin publish split as a single import, skips
│   │   │                           #   duplicates instead of erroring, 400s only if the *new* URLs
│   │   │                           #   alone exceed the caller's remaining quota), images.py (POST /images,
...
│   │   │                           #   contact.py (POST /contact — reachable both logged in and
│   │   │                           #   out via get_optional_current_user; anonymous requires a
│   │   │                           #   verified Turnstile token, logged-in skips it)
...
│   │   │                           #   EmailVerificationToken, PasswordResetToken, EmailJob, Category, Recipe
...
│   │   │                           #   IngredientRefreshJob, ContactMessage
│   │   │                           #   [+submitted_by_user_id — set only when the submitter was
│   │   │                           #   logged in, purely informational])
...
│   │   ├── bookmark_parser.py      # parse_bookmark_links — plain BeautifulSoup <a href> scan of
│   │   │                           #   an uploaded bookmark-export HTML file, deduped by URL,
│   │   │                           #   http(s)-only; no AI call, no DB access
│   │   ├── schemas/                # Pydantic request/response schemas, incl. nutrition.py,
│   │   │                           #   ingredient_refresh.py, contact_message.py
│   │   │                           #   [ContactMessageCreate — a model_validator requires at
│   │   │                           #   least one of email/phone], bookmark.py
│   │   │                           #   [BookmarkParseResponse, BookmarkImportRequest,
...
│   │   │                           #   publish_email_job, publish_contact_message_job
...
│   │                                #   users.last_login_at 0027, recipes.import_reviewed_at +
│   │                                #   import_jobs.error_kind/dismissed_at 0028, import_jobs.
│   │                                #   admin_reviewed_at 0029, import_jobs.category_id nullable
│   │                                #   (AI-suggested, not requester-picked, category) 0030,
│   │                                #   contact_messages table + app_settings.
│   │                                #   contact_recipient_email 0031
│   ├── requirements.txt
...
└── worker/                         # RabbitMQ consumer: fetch -> Claude -> download images -> insert
    ...
    │   │                           #   `recipe_reparse_jobs`, `email_jobs`, AND
    │   │                           #   `contact_message_jobs` — all seven on one connection/channel,
    │   │                           #   same container, see Design Decisions ("Ingredient nutrition")
    │   ├── handlers.py             # handle_import_job — orchestrates fetch -> extract -> resolve
    │   │                           #   Claude's suggested category to a real category_id
    │   │                           #   (_resolve_category_id) -> process images -> insert recipe
    │   │                           #   (owner_user_id from the job's creator, is_shared always
    │   │                           #   False) + one translation row per language; on success, also
    │   │                           #   increments the owner's lifetime imported_recipes_count and
    │   │                           #   creates + publishes a NutritionJob for the new recipe
    ...
    │   │                           #   still builds a category list for the shared extraction
    │   │                           #   prompt but deliberately ignores the returned suggestion —
    │   │                           #   see Design Decisions ("AI-suggested import category")
    │   ├── recipe_sections.py      # SECTION_HEADER_PREFIX ("### ") + is_section_header/
    ...
    │   ├── contact_handlers.py     # handle_contact_message_job — emails a /contact submission to
    │   │                           #   app_settings.contact_recipient_email over SMTP; fails
    │   │                           #   cleanly (not a crash) when that setting is blank. No
    │   │                           #   Reply-To header on the sent message — live-tested against
    │   │                           #   this deployment's real SMTP provider and found it silently
    │   │                           #   rejects the whole send when Reply-To doesn't match the
    │   │                           #   authenticated From address, so the visitor's email is only
    │   │                           #   ever in the body text
    ...
    │   │                           #   (class/style/data-*/on*/aria-*) via BeautifulSoup (lxml
    │   │                           #   backend, not stdlib html.parser — see Design Decisions,
    │   │                           #   "A real import that turned out to be three separate bugs")
    │   │                           #   *before* truncating to max_html_chars, so the char budget
    │   │                           #   goes to recipe content, not boilerplate — see Design
    │   │                           #   Decisions ("Claude cost optimization"); also recovers a
    │   │                           #   JSON-LD Article's articleBody and widens a templated
    │   │                           #   lazy-load image src, same Design Decisions entry;
    │   │                           #   ScrapeDisallowedError
    ...
    │   │                           #   `SUPPORTED_LANGUAGES` entry, plus a best-matching category
    │   │                           #   name chosen from the real, current list given in the prompt
    │   │                           #   (_category_instruction), in the same call — same for
    │   │                           #   parse_recipe_from_text; parse_ingredients_for_nutrition —
    │   │                           #   another tool-use call,
    ...
    │   │                           #   language; every call takes a `provider` ("claude"/
    │   │                           #   "deepseek", default "claude") — DeepSeek reuses the same
    │   │                           #   `anthropic.Anthropic` client pointed at DeepSeek's own
    │   │                           #   Anthropic-compatible base_url, with its own model pair
    │   │                           #   (deepseek-v4-flash/deepseek-v4-pro) — see Design Decisions
    │   │                           #   ("DeepSeek as an alternate AI provider")
    ...
    │   │                           #   otherwise-read-only mirror], ImportJob, Recipe, Category
    │   │                           #   [minimal — id/name/slug, no relationship field, added so
    │   │                           #   handlers.py can resolve Claude's suggested category name],
    │   │                           #   RecipeTranslation, RecipeReparseJob, AppSettings, Ingredient,
    │   │                           #   RecipeIngredientLink, NutritionJob, IngredientRefreshJob,
    │   │                           #   TranslationSyncJob, ContactMessage (mirror the API's
    │   │                           #   models; see Design Decisions)
...
# At least one category must already exist — the worker fails the job immediately if none do
# (see _resolve_category_id), since it has nothing for Claude to pick from.
curl -X POST http://localhost:8000/categories -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" -d '{"name": "Desserts"}'

curl -X POST http://localhost:8000/imports -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" -d '{"source": "https://example.com/some-recipe"}'
# non-admin account: {"id":1,"category_id":null,...,"status":"queued",...}   (queues immediately)
# admin account:      {"id":1,"category_id":null,...,"status":"pending",...} (needs a separate approve)

curl http://localhost:8000/imports/1 -H "Authorization: Bearer $TOKEN"
# with a real ANTHROPIC_API_KEY:    {"id":1,"category_id":1,...,"status":"done","error":null,...}
#   (category_id is whatever Claude picked, resolved against the categories that exist)
# without one:                      {"id":1,"category_id":null,...,"status":"failed","error":"unexpected error: ...auth...",...}
```
...
### 0. Signup, email verification, login, and password recovery
1. A visitor signs up (email/password — no username, see [Design Decisions](#design-decisions),
   "Removing usernames") and solves a Cloudflare Turnstile CAPTCHA (`POST /auth/signup`). The site
   key is fetched from `GET /settings/public` before the visitor has any account, let alone a
   token — it isn't secret, it's the same value embedded in the signup page's own HTML. If the
   email is already registered and verified, the form shows that inline along with a "Forgot your
   password?" link to workflow step 7 below, instead of a dead-end error.
2. The account is created with `is_verified=False` and `language` set to whichever of
   ...
   The worker sends a verification email in that language — explaining what the site is, not just
   a bare link — over the admin-configured SMTP settings, linking back to
   `public_site_url + /verify-email?token=...`. Only "en"/"ro" copy exists in
...
3. `POST /auth/login` (email + password) rejects an unverified account with a specific error, so
   the login form can point at the resend flow instead of just showing "invalid credentials."
4. `POST /auth/resend-verification` (also CAPTCHA-gated, now keyed by email) issues a fresh token
   and re-sends the email — for a lost/expired link, or a first signup that silently failed to send
   because SMTP wasn't configured yet. Signing up again with the same, still-unverified email
   transparently resends instead of erroring on a duplicate.
...
6. `JWT`'s `sub` claim is the account's email (previously username) — `auth.py`'s `AuthUser`
   carries `email` instead of `username` throughout the API.
7. **Forgot password**: `POST /auth/forgot-password` (CAPTCHA-gated) always returns the same
   generic "if that email has an account..." response whether or not the email is registered, to
   avoid leaking which emails are signed up — only a real match actually gets a
   `password_reset_tokens` row (1-hour expiry, single-use) and a `kind="password_reset"`
   `email_jobs` row. `POST /auth/reset-password` consumes the token and sets a new
   `password_hash`, same validation shape (not found/already used/expired) as `verify-email`. See
   [Design Decisions](#design-decisions).
8. **Sign in with Google**: the frontend's `GoogleSignInButton` renders Google's own button (via
   the Google Identity Services script, `clientId` fetched from `GET /settings/public`) and posts
   whatever ID token it gets back to `POST /auth/google`, which verifies the token's signature
   against Google's public keys (no client secret involved) and then, in order: (a) logs straight
   in if `users.google_sub` already matches; (b) else, on an email match, auto-links a verified
   account or claims-and-verifies an unverified one (invalidating its old password — see
   [Design Decisions](#design-decisions)); (c) else creates a new account, but only once
   `terms_accepted` is true — the first call omits it, gets a 400 back, and the frontend shows the
   same `TermsOverlay` component signup already uses before resubmitting the same token with
   `terms_accepted: true`. Every branch returns the same `LoginResponse` shape `POST /auth/login`
   does, so the frontend's `AuthContext.loginWithGoogle` just reuses `login`'s own
   token-persisting logic.
...
1. Any signed-in user submits one URL — including an Instagram post/reel link — from the back
   office (admin) or their own account page (everyone else). No category is picked up front any
   more — see step 4 below and [Design Decisions](#design-decisions), "AI-suggested import
   category". Each becomes an `import_jobs` row owned by that user (`created_by_user_id`),
   `category_id` starting `NULL`.
   - **Admin:** the API still lands the row `status=pending`, **not** yet sent to RabbitMQ — same
     review-before-fetch gate as before. The frontend (`ImportManager.handleSubmit`), though, calls
     **Approve** on it itself the instant creation returns, so from the admin's point of view the
     import just starts — see [Design Decisions](#design-decisions), "Approval gate moved to the
     import request, not the imported result".
...
2. `POST /imports/{id}/approve` publishes a `pending` job to RabbitMQ. A manual **Run import**
   button covering this still exists in the UI (for a job the auto-approve call itself failed to
   reach, and for **Retry** on a `failed` job — a deliberate manual action, not re-automated). The
   UI shows a per-URL status (`pending → queued → fetching → processing → done/failed`), polling
...
4. The worker sends the fetched (and, for HTML sources, cleaned) content to Claude — on
   ...
   language (`SUPPORTED_LANGUAGES`) in the same pass. The same call also asks Claude to pick the
   single best-matching category from the app's real, current category list, given verbatim in the
   prompt (categories are free-text/admin-created, not a fixed enum) — see [Design
   Decisions](#design-decisions), "AI-suggested import category". A sub-group label line (e.g. "For
   the frosting:") comes back prefixed `"### "` instead of being folded into a regular
   ingredient/step — see [Design Decisions](#design-decisions), "Ingredient/step sub-groups".
5. Worker downloads referenced images to local storage, compresses them, and resizes them down to
   a default max dimension/size (configurable in the admin settings). It also resolves Claude's
   returned category name to a real `category_id` (case-insensitive exact match against the list
   sent in step 4, falling back to the first category by id if nothing matches) and writes it onto
   `import_jobs.category_id`. Before inserting anything, it also checks that at least one returned
   translation actually has real content (a non-empty title, and at least one of
   ingredients/steps) — Claude's forced tool-use call has no way to say "there's no recipe here,"
   so a dead URL that now redirects somewhere unrelated (e.g. the original post deleted, now
   bouncing to the site's homepage or the author's Instagram profile) would otherwise come back as
   an empty-but-valid tool call and get saved as a real, blank `APPROVED` recipe. If every
   returned translation is blank, the job fails with `error_kind = not_a_recipe` instead — see
   [Design Decisions](#design-decisions), "Blank recipes from a dead source URL". The same guard
   applies to a reparse (workflow #7), which updates an existing recipe's translations in place
   and would otherwise silently wipe out real, already-published content.
6. Worker inserts the recipe into PostgreSQL already `status = approved`, `owner_user_id` set to
   ...
   [Design Decisions](#design-decisions)), `category_id` from step 5, and `import_reviewed_at =
   NULL`, with one `recipe_translations` row per language, and `added_at`/`approved_at`
   timestamps. In the same transaction, the worker increments the owner's lifetime
   `imported_recipes_count` by one — this is what workflow step 1's limit check reads. Right after
   that commit, the worker automatically queues a nutrition job for the new recipe — see workflow
   #6 below.
7. The owner's Account page surfaces the finished recipe in a "Recently imported" card (filtered
   client-side on `source_url IS NOT NULL AND import_reviewed_at IS NULL`), showing the AI-picked
   category as a small "AI-selected category: {name}" label, until they click **OK, got it**
   (`POST /recipes/{id}/acknowledge-import`, owner-or-admin, 400 on a non-import), which sets
   `import_reviewed_at`. The category itself can be corrected any time from the recipe's own Edit
   page (`/recipes/{id}/edit`), which already has a category selector — no separate approval step
   just for a wrong category. On failure, the worker also sets `import_jobs.error_kind`
   (`disallowed` for a `robots.txt` block, `not_a_recipe` for a technically-successful extraction
   that came back blank, `technical` for everything else) alongside the existing `error` text; the
   same card area shows a canned message keyed off `error_kind` — never the raw
...
**Current state:** all six steps are built end to end for single URLs, Instagram links, and each
individual link a user selects from a bookmark-file import (workflow #3 below) — a bookmark-derived
`import_jobs` row is otherwise indistinguishable from a single-URL one once created. The worker
checks `robots.txt` (`urllib.robotparser`, via `httpx`) for
...
Claude in one `tool_choice`-forced call that returns structured `{category, images, translations:
[{language, title, description, ingredients, steps, tips}, ...]}` — one translation entry per
`SUPPORTED_LANGUAGES` code, plus the single best-matching category name. Each extracted image URL is downloaded, downscaled to
...
Still missing: bulk URL lists (paste several URLs at once) in the single-URL form itself, and the
`settings` table (rate limit/image size/user-agent/etc. are `.env` vars for now, see
[Design Decisions](#design-decisions)).

### 3. Bookmark-file import
1. From the Account page's "Import from bookmarks" panel (a separate section, below the
   single-URL form), the user clicks **Load bookmark file** and picks a browser-exported bookmarks
   HTML file. Nothing is sent anywhere client-side beyond the file itself.
2. `POST /imports/bookmark/parse` reads the file and extracts every `http(s)` link
   (`api/app/bookmark_parser.py`, plain BeautifulSoup `find_all("a", href=True)` — no AI call at
   this step) — deduplicated by URL, `place:`/`javascript:`/other non-http(s) entries dropped.
   Nothing is written to the database yet. The response also flags which of those links are
   `already_imported` (already a `Recipe` or `import_jobs` row for this user) and reports the
   caller's `remaining_quota` — `null` for an admin (unexempt), otherwise
   `max_imports_per_user - imported_recipes_count`.
3. The frontend renders every link as a checkbox — an already-imported one is shown disabled with
   a badge instead of being selectable — and caps how many a non-admin can check at once to
   `remaining_quota`, showing a running "`selected` / `limit` selected" count once they're at the
   cap.
4. Clicking **Import selected** sends the checked URLs to `POST /imports/bookmark`. The endpoint
   re-checks for duplicates server-side (skipped, not an error — reported back as
   `skipped_duplicate`, and **not** counted against the quota, since a duplicate never becomes a
   new import) and re-enforces the remaining-quota limit itself (400 if the caller somehow submits
   more than that many *new* URLs) rather than trusting the frontend's own cap. Each surviving URL
   becomes its own ordinary `import_jobs` row (`type=bookmark`) and goes through the exact same
   fetch → Claude(or DeepSeek)/clean/translate → download-images pipeline as a single-URL import,
   with the same admin-vs-non-admin behavior: a non-admin's jobs queue and publish immediately, an
   admin's own still land `pending` for their usual approval click. A short result message reports
   how many imports started and how many were skipped as duplicates.
5. From there it's indistinguishable from a single-URL import: it shows up in the same "in
   progress" list (`ImportManager`, which now also reloads on an externally-created batch — see
   [Design Decisions](#design-decisions), "Bookmark import"), and on success lands in the same
   "Recently imported" review panel as workflow #2's step 7.

A **How do I export my bookmarks?** link next to the panel opens a scrollable overlay with
step-by-step instructions for Chrome, Firefox, Safari, and Internet Explorer, closed by a single
OK button.
...
1. A super admin opens `/backoffice/users` (new **Users** tab, alongside Recipes/Settings) to see
   every account in one table: email, confirmed/unconfirmed, joined date, last login,
...
3. That list is powered by the existing `GET /recipes` endpoint's `owner` filter, extended to
   accept an explicit user id (previously only the literal `"me"`) for an admin caller — 403s a
   non-admin, 404s an unknown id — rather than adding a second, parallel listing endpoint.
...
### 9. Contact page
1. Any visitor opens `/contact` — reachable whether signed in or not, and always in the header nav
   regardless of auth state. Fields: name (required), email and phone (each optional
   individually, but at least one of the two is required — enforced by a Pydantic
   `model_validator` on `ContactMessageCreate`, not just client-side), and message (required,
   minimum 50 characters).
2. If the visitor isn't signed in, they must also solve a Turnstile CAPTCHA (same widget/flow as
   signup) — `POST /contact` verifies it server-side via `get_optional_current_user` +
   `verify_turnstile` and 400s without a valid one. A signed-in visitor skips this entirely; the
   form doesn't even fetch the Turnstile site key in that case.
3. On success, the API inserts a `contact_messages` row (`status=queued`, `submitted_by_user_id`
   set only when the submitter was logged in) and publishes a `contact_message_jobs` message —
   same fire-and-forget-but-durable pattern as every other job type: a broken queue publish is
   recorded on the row (`status=failed`) but doesn't turn the already-successful submission into a
   user-facing error.
4. The worker (`contact_handlers.py`) sends the submission to
   `app_settings.contact_recipient_email` (a plain, admin-configurable Settings field — not
   secret, but not exposed via `GET /settings/public` either) over the same SMTP config used for
   verification emails. A blank `contact_recipient_email` fails the job cleanly with an actionable
   error rather than crashing. The visitor's own email/phone are in the message body, not a
   `Reply-To` header — see [Design Decisions](#design-decisions), "Contact page", for why.

## Data Model (sketch)

- **users** — id, email (unique, `NOT NULL` — the account's only identifier now; see
  [Design Decisions](#design-decisions), "Removing usernames". Migration `0033` dropped the old
  `username` column and backfilled the one pre-existing account that lacked an email, the
  migration-seeded original admin, by hand before deploying),
  ...
  the signup form at signup time, used only to pick which language the verification/password-reset
  email is sent in; not kept in sync with the user's browsing language afterward, since there's
  currently no other account email that would need it), imported_recipes_count (lifetime count of
  *successful* imports, default 0, incremented once by the worker per import and never
  ...
  column, see [Design Decisions](#design-decisions)), google_sub (nullable, unique — Google's
  stable per-account subject id, set once this account signs in via Google; Postgres allows
  unlimited `NULL`s in a unique index, so this needs no special handling for the majority of
  accounts that never set it. Added by migration `0034`, see
  [Design Decisions](#design-decisions), "Sign in with Google")
...
- **password_reset_tokens** — id, user_id (FK → users.id, `ON DELETE CASCADE`), token (unique,
  random), expires_at (1 hour from creation — shorter than a verification token's 24h, since a
  reset link directly grants account access), used_at (nullable), created_at. Structurally
  identical to `email_verification_tokens`, kept as a separate table rather than a shared one so
  the two token kinds can't be confused/reused across flows. Added by migration `0033`
- **email_jobs** — id, user_id (FK → users.id), kind (`"verification"` or `"password_reset"`),
  status (`queued` \| `processing` \| `done` \| `failed`), error, created_at. Queued by
  `POST /auth/signup` / `/resend-verification` / `/forgot-password`, sent by the worker over the
  admin-configured SMTP settings
- **recipe_favorites** — id, user_id (FK → users.id), recipe_id (FK → recipes.id), created_at;
  unique on (user_id, recipe_id). Read two ways: `GET /users/me/favorites` (unpaginated, used
  nowhere in the frontend any more) and `GET /recipes?favorites_only=true` (paginated, combinable
  with `category_id`, powers the recipes page's Favorites filter — see
  [Design Decisions](#design-decisions), "Splitting Import out of the account page"), both
  excluding a favorite whose recipe has since lost visibility to the caller (same
  `_visibility_clause` as everywhere else)
- **recipe_shares** — id, recipe_id (FK → recipes.id, `ON DELETE CASCADE`), token (unique, random,
  `secrets.token_urlsafe(24)`), created_by_user_id (FK → users.id), created_at, expires_at (24h
  from creation), revoked_at (nullable — set by the owner canceling an outstanding link early; a
  soft delete, not a row delete, so `GET /recipes/{id}/shares` can still show it as revoked rather
  than it just disappearing). Added by migration `0037`. Status (`active`/`expired`/`revoked`) is
  computed from these two timestamps at request time, never stored. Deliberately multi-use, not
  single-claim: any number of people who have the token can each independently view or copy — see
  [Design Decisions](#design-decisions), "Recipe sharing". The token itself is the sole
  authorization to see this specific recipe regardless of its normal visibility (private, or
  shared-but-pending-approval) — `GET /recipe-shares/{token}` and its `/copy` sibling bypass
  `_visibility_clause` entirely, by design
...
- **recipes** — id, category_id (owner-or-admin editable via `PATCH /recipes/{id}/category` — see
  ...
  "Post-import review panel"), shared_from_recipe_id (nullable FK → recipes.id, `ON DELETE SET
  NULL` — provenance only, set once at creation when this recipe was produced by copying a
  `recipe_shares` link; see below) — language-independent fields only; text content lives in
...
- **recipe_translations** — id, recipe_id (FK → recipes.id), language (e.g. `ro`, `en` — a plain
  ...
  see [Design Decisions](#design-decisions), "Ingredient/step sub-groups". `title` also carries a
  trigram GIN index (`ix_recipe_translations_title_trgm`, on `lower(f_unaccent(title))`, via the
  `pg_trgm` and `unaccent` extensions — migration `0036`) powering `GET /recipes?search=...`'s
  diacritic-insensitive substring title filter; `f_unaccent` is a small `IMMUTABLE` SQL wrapper the
  same migration creates around `unaccent()` (which is only `STABLE` on its own, so can't be used
  in an index expression directly) — see [Design Decisions](#design-decisions), "Recipe search"
- **import_jobs** — id, category_id (nullable — null until the worker resolves it from Claude's
  own suggestion once extraction succeeds; the requester no longer picks one up front, see
  [Design Decisions](#design-decisions), "AI-suggested import category"; the recipe it produces on
  success inherits the resolved value), created_by_user_id (FK →
  users.id — who created the request; the recipe it produces inherits this as `owner_user_id`),
  type (`single` \| `instagram` \| `bulk` \| `bookmark` — `bookmark` is set by
  `POST /imports/bookmark` for each link selected from a bookmark-file import, see [Core
  Workflows](#core-workflows) #3; `bulk` remains unreachable, no pasted-URL-list form exists yet),
  source, status
  (`pending` \| `queued` \| `fetching` \| `processing` \| `done` \| `failed`), error, error_kind
  (nullable — `disallowed` \| `not_a_recipe` \| `technical`, set alongside `error` on failure; a
  non-admin caller only ever receives this categorized kind, never the raw `error` text.
  `not_a_recipe` means extraction technically succeeded but came back entirely blank — see [Design
  Decisions](#design-decisions), "Blank recipes from a dead source URL"), created_at,
...
- **app_settings** — a single row (id=1), not a real multi-row table: supported_languages,
  ...
  contact_recipient_email (default `""` — where a `/contact` submission is emailed; not secret,
  but also not exposed via `GET /settings/public`, since nothing on the pre-login side needs it),
  preferred_ai_provider (default `"claude"`, the other valid value is `"deepseek"` — which provider
  `worker/app/claude_client.py` calls for extraction/parsing/translation), deepseek_api_key
  (default `""` — secret, follows the same masked/blank-means-unchanged convention as
  anthropic_api_key; see [Design Decisions](#design-decisions), "DeepSeek as an alternate AI
  provider"), google_client_id (default `""` — a Google Cloud OAuth Web Client ID, gating "Sign in
  with Google"; not secret, same reasoning as turnstile_site_key, so it's readable via
  `GET /settings/public`. Added by migration `0034`). Seeded once from `.env` by migration `0006`/`0007`/`0017`; editable live from the back office
...
- **contact_messages** — id, name, email (nullable), phone (nullable — at least one of email/phone
  is required, enforced by `ContactMessageCreate`'s `model_validator`, not a DB constraint),
  message, submitted_by_user_id (FK → users.id, `ON DELETE SET NULL`, nullable — set only when the
  submitter was logged in, purely informational), status (`queued` \| `processing` \| `done` \|
  `failed`), error, created_at. Queued by `POST /contact`, emailed by the worker to
  `app_settings.contact_recipient_email`. No admin viewer page for these — the notification email
  itself is the record; `psql` is the only way to look one up after the fact
...
- **recipe_ingredient_links** — id, recipe_id (FK → recipes.id), ingredient_index (position in
  ...
  estimated_grams, grams_source (`api_lookup` \| `claude_estimate`); indexed but **not** unique on
  (recipe_id, ingredient_index) — a single source line can legitimately name more than one food
  (e.g. "salt, black pepper"), so two rows sharing an index is real data, not a duplicate. The only
  nutrition data actually stored — see [Design Decisions](#design-decisions), "Ingredient
  nutrition"
...
`scripts/backup.sh` backs up the two things that actually hold durable state — Postgres (all
structured data) and the `images_data` volume (recipe photos) — into dated, compressed files
under `./backups/db/` and `./backups/images/` in the deployed repo checkout.
`rabbitmq_data` is deliberately not backed up: it only ever holds transient in-flight job state,
nothing that isn't already reconstructible.

Set it up on the VPS with a daily cron entry (pick whichever hour; low-traffic is safest):

```cron
0 3 * * * /path/to/cookbook/scripts/backup.sh >> /var/log/cookbook-backup.log 2>&1
```

The script reads `POSTGRES_USER`/`POSTGRES_DB` from the already-running `db` container's own
environment rather than parsing `.env` itself on the host — `.env` has unquoted values containing
spaces/parens (`SCRAPE_USER_AGENT`) that aren't valid to `source` as a shell script. It streams
`pg_dump` and a `tar` of `/data/images` straight out of the already-running `db`/`api` containers
(`docker compose exec`), so no extra containers or knowledge of the volume's actual Docker-assigned
name is needed.

**Retention**: today's and yesterday's backups are always kept; on top of that, the single oldest
survivor aged 2–5 days represents the "~4 days ago" slot and the single oldest survivor aged 6–9
days represents the "~8 days ago" slot — so at most 4 backups of each kind exist at any time. Each
older slot's occupant keeps winning against younger challengers (it stays the "oldest in range")
until it ages out of its range, at which point the slot goes briefly empty and gets re-seeded by
whatever next reaches that age — see [Design Decisions](#design-decisions), "Local backup
strategy", for why this has to work by picking the oldest survivor in a range rather than matching
an exact age. This is a local-only, on-VPS backup; it does not protect against losing the VPS
itself. Getting a copy safely off the server (e.g. an external tool pulling `./backups/` via
`rsync`/`scp` on its own schedule) is intentionally left to whatever the operator already has
for that, rather than baking a specific offsite destination/credential into this repo.

**Restore**:

```bash
# Database — replaces everything currently in the DB, so stop api/worker first if restoring
# into a live stack, not just a fresh one.
gunzip -c backups/db/cookbook-db-YYYYMMDD.sql.gz \
  | docker compose -f docker-compose.yml -f docker-compose.prod.yml exec -T db \
      sh -c 'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB"'

# Images — extracts on top of whatever's already in the volume; wipe /data/images first for a
# clean restore instead of a merge.
gunzip -c backups/images/cookbook-images-YYYYMMDD.tar.gz \
  | docker compose -f docker-compose.yml -f docker-compose.prod.yml exec -T api \
      tar xzf - -C /data/images
```
...
|-------------------|-------------------------------------|--------|-------|
| API backend        | `pytest` + FastAPI `TestClient`    | 329 tests, passing | Categories/recipes CRUD, validation (unknown category, duplicate slug), status transitions, `GET /languages`, translation language resolution/fallback, recipe approve (sets `approved_at`, 404), rejects a whole-field URL in title/description/ingredients/steps/tips (400) while allowing normal text that merely contains "http". `test_recipe_visibility.py`: an anonymous/other user only sees a recipe if `approved` + `is_shared`, the owner always sees their own regardless of status, an admin sees everything, `GET /recipes/{id}` 404s (not 403s) on an invisible recipe, `PATCH /recipes/{id}/share` succeeds for the owner or an admin, 403s for anyone else, 400s on an imported recipe, and re-sets `status=unapproved` when a non-admin turns sharing on. Import jobs (`test_imports.py`): create stays `pending` for an admin and doesn't publish, queues and publishes immediately for a non-admin, approve publishes and moves to `queued`, approve retries a `failed` job, approve rejects an already-`queued` job (400), duplicate-URL rejection is scoped per creator (409, two different users can import the same URL independently), a non-admin only sees/modifies their own jobs while an admin sees everyone's, `created_by_user_id` is always populated and `created_by_email` only for an admin caller, `category_id` starts `null` on creation (no longer part of `ImportJobCreate` — an extra `category_id` in the POST body, still accepted by some other clients, is just ignored). `test_auth.py`/`test_users.py`: signup requires a valid Turnstile token, rejects a verified duplicate email, resends instead of erroring on a duplicate unverified email, records the submitted `language` on the new user and falls back to `default_language` when it's missing or not in `supported_languages`, login (by email, no username) rejects an unverified account with a specific message, `/verify-email` accepts a valid token once and rejects an expired/reused/unknown one, `/resend-verification` re-sends and is itself CAPTCHA-gated, `GET /users/me` and `PATCH /users/me/password` require a valid current password. Password recovery (`test_auth.py`): `/forgot-password` requires a valid Turnstile token and returns the exact same generic response for a registered vs. unregistered email while only creating a `password_reset_tokens` row for the real one, `/reset-password` succeeds and immediately invalidates the old password (confirmed via a follow-up login with both), rejects an invalid/reused/expired token (400 each). Google sign-in (`test_auth.py`, `verify_oauth2_token` mocked): a new account requires `terms_accepted` first (400, nothing created) then succeeds on retry; a second call with the same `google_sub` logs straight in even if Google's own email changed; auto-links a verified existing account by email; claims an unverified one (verifies it, links it, and invalidates its old password, confirmed via a follow-up `POST /auth/login`); rejects an invalid token, an unverified Google email, and an unconfigured `google_client_id` (blank); stamps `last_login_at`. `google_client_id` round-trips through `PATCH`/`GET /settings` and is exposed via `GET /settings/public` alongside the Turnstile site key. Settings: `GET`/`PATCH /settings` require **super**-admin (a plain admin is rejected), secrets (including `calorie_ninjas_api_key`, `smtp_password`, `turnstile_secret_key`) never appear in the response body, numeric fields reject non-positive values (422), invalid/empty language codes and a `default_language` outside `supported_languages` are rejected (400), an omitted/blank secret leaves the current value untouched, partial updates don't clobber other fields; `GET /settings/public` returns only the Turnstile site key, unauthenticated. Nutrition: `GET /recipes/{id}/nutrition` is public (no `POST` here — enrichment is triggered by the worker, not the API) and reports `not_enriched` for a fresh recipe, 404 for an unknown one, computes totals/per-serving/per-ingredient-grams/`total_grams` correctly from `recipe_ingredient_links` × `ingredients`, and keeps showing previously-good totals even while a re-enrichment is queued/processing or after it fails. Recipe editing: `PUT /recipes/{id}` enqueues a `translation_sync_jobs` row only when a translation field actually changed, `processing_status` reflects whichever of nutrition/translation-sync is currently active for a recipe. Ingredient refresh: `GET`/`POST /ingredients/refresh` both require admin, `GET` reports `never_run` before any job exists and otherwise the latest job (tie-broken by id, not just timestamp), `POST` queues and publishes. Import limit (`test_imports.py`): `POST /imports` 403s a non-admin at/over `max_imports_per_user` with the current/limit counts in the message, allows the request one under the limit, exempts admins even far over it, and the counter is untouched by an admin deleting the resulting recipe; `GET /users/me` exposes `imported_recipes_count`. Recipe search (`test_recipes.py`): `GET /recipes?search=...` matches a title substring case-insensitively, requires every word in a multi-word query (any order, so "chicken soup" also matches "Soup with Chicken"), scopes matching to the requested/default `language` (a match in the other language's translation doesn't leak through), rejects a query under 3 characters (422), composes with `category_id` (AND, not OR), treats a literal `%`/`_` in the query as plain characters rather than SQL wildcards, and ignores diacritics in both directions ("briose" matches "Brioșe cu dovleac...", and "tocăniță" matches a plain-ASCII "Tocanita simpla de vinete"). Reparse (`test_recipes.py`): `POST /recipes/{id}/reparse` and `/reparse-all-imported` both require admin and 400 on a recipe with no `source_url`. Users page (`test_users.py`): `GET /users` requires super-admin (401 unauthenticated, 403 a plain admin), reports correct owned/shared/imported counts including a user with zero recipes, `POST /auth/login` stamps `last_login_at`. `GET /recipes`'s admin-only `owner` lookup (`test_recipe_visibility.py`): returns the target user's recipes in any status, 403s a non-admin or anonymous caller, 404s an unknown id, 400s a non-numeric value. Owner-editable category (`test_recipe_visibility.py`): `PATCH /recipes/{id}/category` succeeds for the owner (manually-added or imported recipe alike) or an admin, 403s a non-owner non-admin, 401s an anonymous caller, 400s an unknown category id. Owner-editable delete (`test_recipes.py`): `DELETE /recipes/{id}` succeeds for the owner or an admin, 403s a non-owner non-admin (the recipe still exists afterward), 401s an anonymous caller. Deleting an imported recipe also frees up its source URL for re-import (`test_imports.py`): `POST /imports` for a URL that was already imported and then deleted succeeds (201) instead of permanently 409ing, confirmed to actually fail without the cleanup by reverting the fix and re-running the test. Favorites filter (`test_recipe_visibility.py`): `GET /recipes?favorites_only=true` returns only the caller's favorited recipes, combines with `category_id` to narrow further, excludes a favorite whose recipe has since lost visibility (e.g. un-shared after favoriting), and 401s an anonymous caller. Post-import review panel (`test_recipe_visibility.py`'s `TestUpdateEndpoint`/`TestAcknowledgeImportEndpoint`, `test_imports.py`): `PUT /recipes/{id}` now succeeds for the owner, 403s a non-owner non-admin, 400s a non-admin's `status` change, still 401s unauthenticated; `POST /recipes/{id}/acknowledge-import` succeeds for the owner or an admin, 403s a non-owner non-admin, 400s a non-imported recipe; `POST /imports/{id}/dismiss` succeeds for the owner or an admin and sets `dismissed_at`, 400s a non-`failed` job, 404s a non-owner non-admin; `GET /imports` with `limit`/`offset` paginates and sets `X-Total-Count`, filters by `status`, and omitting `limit` keeps returning the full unheadered list; a non-admin caller never receives the raw `error` text (only `error_kind`) even for their own job, while an admin sees both. Admin review tracking (`test_imports.py`): `POST /imports/{id}/mark-reviewed` requires admin (403 otherwise), sets `admin_reviewed_at` without touching `dismissed_at`, 400s a job that hasn't failed; the owner's own `/dismiss` afterward leaves `admin_reviewed_at` untouched (and vice versa); `GET /imports`'s `admin_reviewed` filter excludes/includes reviewed jobs correctly. Contact form (`test_contact.py`): anonymous submission 400s without a `turnstile_token` and 400s an invalid one, succeeds with a valid one; a logged-in submitter skips the CAPTCHA check entirely and gets `submitted_by_user_id` set; rejects a payload with neither email nor phone (422) while accepting either alone; rejects a message under 50 characters and a blank name (422); a broken queue publish still returns 201 but leaves the row `status=failed` with the publish error recorded. Settings (`test_settings.py`): `contact_recipient_email` defaults to `""`, round-trips through `PATCH`/`GET`; `preferred_ai_provider` defaults to `"claude"` and `deepseek_api_key_is_set` to `false`, updating both round-trips through `PATCH`/`GET` with the raw key masked, an unrecognized provider value is rejected (422), and a blank `deepseek_api_key` leaves the current value untouched. `test_bookmark_parser.py`: extracts only `http(s)` links (drops `javascript:`/`place:`/other browser-internal entries), dedupes by URL keeping the first title seen, falls back to the URL itself when link text is blank, preserves file order, empty for a linkless file, caps at 1000 links. Bookmark import (`test_imports.py`): `POST /imports/bookmark/parse` returns the candidate links (filtered the same way), reports `remaining_quota: null` for an admin and the correct remaining count for a non-admin, flags a link as `already_imported` whether it matches an existing `Recipe.source_url` or an `ImportJob.source` for that caller, 401s unauthenticated; `POST /imports/bookmark` creates one job per selected URL (type `bookmark`), admin jobs stay `pending` while a non-admin's queue and publish immediately (same split as a single-URL import), rejects a selection whose *new* URLs alone exceed the caller's remaining quota (400) while allowing exactly the remaining amount, exempts admins from that check entirely, skips a URL that's already a recipe/queued job instead of erroring — reported back in `skipped_duplicate` and not counted against the quota check — dedupes a URL repeated within the same request, 401s unauthenticated. `test_url_normalize.py`: strips a `?igsi=...` tracking param and adds a missing trailing slash for `instagram.com`/`www.instagram.com`/`instagr.am` hosts, leaves the query string and any other host untouched, trims whitespace regardless of host. Instagram URL normalization (`test_imports.py`): a re-shared link (a different `igsi` token, same reel) to an already-imported/queued reel is rejected as a duplicate (single-URL and bookmark-parse `already_imported`), two share-link variants of the same reel selected in one bookmark batch collapse to a single created job with the rest reported in `skipped_duplicate`, and the stored `source` is the normalized (query-stripped) form regardless of what was submitted. Recipe sharing
(`test_recipe_shares.py`): `POST /recipes/{id}/shares` requires owner-or-admin and returns a token
expiring ~24h out; `GET /recipe-shares/{token}` reports `active`/`expired`/`revoked` correctly
(expiry/revocation backdated directly in the DB), an unauthenticated caller gets the teaser
(title/image/160-char-truncated description) but not the full recipe or sender email, an
authenticated one gets both; `POST /recipe-shares/{token}/copy` requires auth, 410s an
expired/revoked link, and produces a fully independent `Recipe` (new id, new slug,
`shared_from_recipe_id` set, its own physically-duplicated image files on disk — confirmed via
`tmp_path` that the *original's* photo survives deleting the original recipe afterward — its own
`RecipeIngredientLink` rows so nutrition is available immediately, no re-enrichment wait); the same
link copied by two different users succeeds independently for each (multi-use, not single-claim);
revoke is owner-or-admin only and idempotent |
| Worker              | `pytest`                           | 175 tests, passing | `scraping.fetch_page` (robots.txt allow/disallow/missing/unreachable, truncation *after* HTML cleaning not before, strips script/style/nav/footer/header/aside/form/iframe/svg/comments and noisy attributes while preserving `<img src>` and real content, promotes `data-src`/`data-lazy-src`/`data-original`/`data-srcset` into a real `src` for lazy-loaded images without clobbering an already-real one, keeps a `<noscript>` image fallback intact, recovers a JSON-LD `Article`/`BlogPosting`/`NewsArticle` block's `articleBody` for a page whose real content isn't reachable in the parsed HTML at all, preferring the longest one when a page embeds more than one near-duplicate block and ignoring a non-Article type or malformed JSON entirely, widens a templated lazy-load `data-src` (a `width=` query param pointing at a placeholder size, e.g. `width=1`) to the largest value in a sibling `data-widths` list rather than promoting the tiny placeholder as-is, leaves a normal (non-templated) lazy `src` alone, resolves a Google AMP-viewer or `google.<tld>/url?q=` redirect-notice link to its real destination (returned as the new `final_url`) with a fresh robots.txt + rate-limit check against *that* domain rather than google.com's own (which it deliberately skips — see `_is_google_redirect_url`), a page that merely lives at a path called `/url` on a non-Google host is left alone, HTTP errors, current DB-backed rate limit reaches the rate limiter on every call), `rate_limiter.DomainRateLimiter` (per-domain interval, crawl-delay override, a per-call rate override taking precedence over the constructor default, deterministic via injected clock), `claude_client.extract_recipe` (multi-language tool-use response parsing, the given API key reaches the Anthropic client, runs on `EXTRACTION_MODEL`/Haiku, requests a high enough `max_tokens` for a long recipe in multiple languages, includes the given category list verbatim in the prompt so Claude can pick one — same for `parse_recipe_from_text`), `claude_client.parse_ingredients_for_nutrition` (structured food/quantity/unit/grams parsing, also Haiku), `claude_client.parse_recipe_from_text` (Haiku, same `max_tokens` floor), and `claude_client.translate_recipe` (re-translates edited content into every other configured language, creating a missing language's translation the same way a fresh import would, stays on `MODEL`/Sonnet, same `max_tokens` floor); every one of those four functions also verified for its `provider` argument — omitted, it still resolves to Claude's own models; `provider="deepseek"` selects `DEEPSEEK_EXTRACTION_MODEL`/`DEEPSEEK_MODEL` and reaches `_client`, which points `anthropic.Anthropic` at DeepSeek's own Anthropic-compatible `base_url` instead of the default (not just the model name — a regression test locks in that the provider argument actually reaches client construction, not only model selection), and passes `thinking={"type": "disabled"}` for DeepSeek only, never for Claude, and its shared prompt text explicitly forbids turning a page's own whole-list heading into a sub-group marker and forbids carrying over the source's own step numbers (see Design Decisions, "DeepSeek as an alternate AI provider"), `images.process_images` (resize to max dimension, quality-reduction loop respects/floors on max size, skips failed downloads/non-image content without failing the batch), `test_instagram_client.py` (Playwright drives a headless page load and extracts caption/comments/preview image; a failed/blocked load is handled without crashing the job), `handlers.handle_import_job` (success inserts an already-`approved` recipe owned by the import job's creator with `approved_at` set and one translation row per language, stores `fetch_page`'s resolved URL as `source_url` — not necessarily `job.source` itself, e.g. after a Google redirect-notice link resolves to the real article — and joins a relative image URL against that same resolved page rather than the originally-submitted link, *and* auto-enqueues + publishes a `NutritionJob` for it, *and* increments the owner's `imported_recipes_count` by exactly one; a broken queue publish for that follow-up job marks the `NutritionJob` failed without touching the already-successful import job; each import failure mode, including an empty `translations` list, maps to a distinct `job.error` *and* the correct `error_kind` (`disallowed` only for a `robots.txt` block, `not_a_recipe` when every returned translation is blank — e.g. a dead source URL that now redirects somewhere unrelated, see [Design Decisions](#design-decisions), "Blank recipes from a dead source URL" — `technical` for everything else, Instagram included) without leaving a half-inserted recipe, and does *not* increment the count; succeeds normally when only *some* fields on a translation are blank, since that's a legitimately thin recipe rather than a dead page; also resolves Claude's suggested category name to a real `category_id` by case-insensitive exact match, falls back to the first category by id when the suggestion matches nothing, and fails cleanly with an actionable message when no category exists at all), `reparse_handlers.handle_reparse_job` (updates existing `recipe_translations` rows in place rather than inserting a new `Recipe`, leaves slug/images/owner untouched, re-enqueues nutrition on success, fails cleanly on a scrape/extraction error, and fails with `NotARecipeError` without touching the existing recipe at all when the re-extraction comes back entirely blank — same guard and same reasoning as the import path), `translation_sync_handlers.handle_translation_sync_job` (translates the edited language into every other one, re-enqueues a `NutritionJob` on success, fails cleanly on an unknown recipe/job or an unexpected exception, still succeeds even if the nutrition re-enqueue itself raises), `email_handlers.handle_email_job` (sends a real SMTP message using the admin-configured host/port/credentials/TLS setting, dispatches on `job.kind` — builds the verification link or the password-reset link from `public_site_url` accordingly, looking up the newest unused token of the matching kind — sends the user's own `language` copy for either kind and falls back to English for an unrecognized language, fails cleanly on an unknown `kind` or no matching unused token), `consumer._on_message` (delegates and acks; marks a job `failed` with the exception message if a handler raises *anything* unexpected, even with an unparseable body), `settings_service.get_settings` (falls back to defaults when the row is missing, reads the current row, reflects an update on the very next call — no cache) — including `SettingsSnapshot.ai_api_key`, which resolves to `anthropic_api_key` by default and to `deepseek_api_key` once `preferred_ai_provider` is `"deepseek"`. `nutrition_api_client.lookup_nutrition`/`extract_nutrients_per_100g` (returns the first matching item, sends the `X-Api-Key` header, returns `None` for an empty result, normalizes nutrients to a per-100g basis from whatever `serving_size_g` came back, defaults to zero when the serving size is missing/zero). `nutrition_handlers.handle_nutrition_job` (creates a new `Ingredient` + `RecipeIngredientLink` on success, computes `estimated_servings` from total resolved grams ÷ a reference portion weight floored at 1 (not a separate Claude guess) — `STANDARD_PORTION_GRAMS` by default, or a smaller category-specific weight from `_PORTION_GRAMS_BY_CATEGORY` (e.g. Desserts) when the recipe's category has one, see [Design Decisions](#design-decisions), "Estimated servings" — prefers a CalorieNinjas gram figure over Claude's own estimate when one matches *and* the unit isn't a confirmed-unreliable one, skips the CalorieNinjas refinement call entirely for `ml`/`gr`/`l`/`liter(s)`/`litre(s)` and trusts Claude's estimate directly, reuses an existing `Ingredient` by name without a redundant lookup, rounds a decimal quantity like `5.5` to `6` before querying to work around a CalorieNinjas parser bug that mangles decimals, a malformed ingredient item is skipped without failing the rest, fails cleanly with no ingredients / no parsed items / an unknown recipe / an unexpected exception, replaces (not accumulates) links from a previous run, and allows two returned items to share the same `line_index` — e.g. "salt, black pepper" parsed as two foods on one source line — now that `recipe_ingredient_links` no longer enforces uniqueness on it). `ingredient_refresh_handlers.handle_ingredient_refresh_job` (refreshes an ingredient via a `"100g <name>"` lookup, leaves an ingredient untouched when nothing matches, one ingredient's CalorieNinjas failure doesn't sink the others, fails cleanly with a clear message when no CalorieNinjas key is configured, ignores an unknown job id). `contact_handlers.handle_contact_message_job` (sends a real SMTP message to `contact_recipient_email` with the visitor's name/email/phone/message in the body, confirms no `Reply-To` header is ever set — see Design Decisions, "Contact page" — fails cleanly with an actionable error when `contact_recipient_email` is blank, fails cleanly on a real SMTP error, ignores an unknown job id) |
| Frontend            | Vitest + React Testing Library     | 229 tests, passing | `useRecipeSearch` (fake timers): stays `undefined` until 1.5s after typing stops, never activates under 3 characters even once the debounce settles, and resets the debounce timer on every keystroke. `matchesSearchWords` (`searchMatch.test.ts`): case-insensitive substring match, requires every word in any order, and matches regardless of which side (title or query) has diacritics. Recipe search boxes: `RecipeBrowser` and `RecipeManager` send the debounced `search` value to `listRecipesPage` 1.5s after typing 3+ characters and show the new no-results text in place of the grid/list on an empty response (`RecipeManager` also resets to page 1); `ImportPage`'s search filters the already-loaded submissions list client-side with the same debounce/3-character rule (including diacritic-folding), leaving `submissions` itself untouched and showing the no-results text only when the filter (not the underlying list) is empty. `CategoryManager`/`RecipeManager`: list/create/delete, error handling, approve button, preview toggle, full per-language edit form incl. image upload. `ImportManager`: submits just a URL (no category picker any more — the AI suggests one after extraction, see workflow #2), approve button gated to `pending`/`failed` jobs only, delete, shows a job's error, shows who created a job (`imported by {email}`, admin-only — a non-admin's own job shows just the id-based ownership check with no email in the response), explains that imports are private to the importer, polls every 3s while any job is `queued`/`fetching`/`processing` and stops once settled, calls `onJobCreated` after a successful create and after a successful approve; auto-approves a freshly created job that lands `pending` so it starts fetching right away with no extra click (an admin's, per workflow #2), and does not call approve for one that's already `queued` (a non-admin's); lifetime import limit: an admin never fetches or shows usage, a non-admin sees "`used` / `limit` imports used", the Add button is blurred+disabled once the limit is reached (and `handleSubmit` itself also guards on it, not just the `disabled` attribute, verified via a direct form-submit event); reloads the job list when an external `reloadTrigger` prop changes, so a job created elsewhere (bulk bookmark import) shows up without a manual action here. `BookmarkImportPanel`: shows the parsed links as checkboxes after loading a file, renders an already-imported link disabled with a badge and excludes it from selection, caps selection at a non-admin's remaining quota (disabling further boxes and showing a running count) while showing no cap at all for an admin, imports the selected links and reports a created/skipped summary while notifying the parent via `onJobCreated`, surfaces a parse error and lets the admin retry, resets back to the load-file state via "Choose another file", opens and closes the how-to-import dialog. `HowToImportBookmarksDialog`: shows export steps for Chrome/Firefox/Safari/Internet Explorer, closes via the OK button or Escape. `RecipeBrowser`: cards link to `/recipes/:id`, category filter triggers a re-fetch, splits into "My recipes"/"From the community" sections when logged in vs. one public feed when logged out (never double-counting the viewer's own shared recipe in the community section), loads more on scroll via the sentinel, shows/hides the favorite toggle based on auth state and saves it via the API. Favorites filter: hidden while logged out, shows the "Add or import recipes" button (linking to `/import`) only while logged in, toggling the Favorites pill replaces the mine/community split with one flat list (heading changes to "Your favorites", showing an empty state when there are none, no "From the community" section) and combines with a selected category in the same request, toggling it back off restores the normal split. `RecipeDetail`: renders full content once loaded, blocks unapproved recipes with a message, surfaces fetch errors. `PublicRecipeDetail`'s JSON-LD: includes `recipeYield` and a `NutritionInformation` block (calories/protein/carbs/sugar/fat, all per serving) once nutrition enrichment is `done`, omits both while it isn't. `LanguageProvider`: defaults to Romanian, reads/persists a stored language, switching updates translations live. `AuthProvider`: starts unauthenticated, reads a stored token (and role flags) on mount, login success persists the token and flips state, login failure leaves it unauthenticated, logout clears both. `LoginForm`: submits entered email/password, shows the translated error message on failure, including the "not verified yet" case with a resend option, links to `/forgot-password`. `SignupForm`: submits the current UI language alongside the form fields once terms are accepted, shows a "Forgot your password?" link instead of a dead-end error when the server reports the email is already registered. `ForgotPasswordPage`: submits the email once the CAPTCHA is solved and shows the server's generic response, requires the CAPTCHA first, shows the server's error on failure. `ResetPasswordPage`: submits the new password with the token read from the URL, shows an error and disables submit when the link has no token, flags a too-short password or a mismatched confirmation before submitting, shows the server's error on a failed reset (expired/reused/invalid token). `GoogleSignInButton` (stubbing `window.google.accounts.id`): renders nothing with a blank `clientId`, initializes and renders Google's own button once the script is available, calls `loginWithGoogle` with the credential on a successful sign-in, shows the shared `TermsOverlay` and resubmits with `terms_accepted: true` on the terms-required error, hands any other failure to `onError`. `SettingsManager`: loads and displays current settings including SMTP/Turnstile/`max_imports_per_user` fields, secret fields (including the new DeepSeek key) stay blank with a placeholder hinting whether one is set, Apply omits a secret from the request when left blank but includes it when typed, saves a changed `max_imports_per_user`, defaults the AI provider select to the current setting and saves a changed one, shows a success message on save and the server error on failure. `NutritionPanel`: renders per-serving values + the servings badge when both are known, shows the total weight badge in grams under 1kg and kilograms above it, hides that badge when `total_grams` is unknown, falls back to whole-recipe totals when there's no servings estimate, renders nothing when nutrition isn't enriched yet or a job is still in flight. `IngredientRefreshPanel`: shows "never run" on first load, creates a job and shows its status on click, polls while active and shows the updated count once done (and stops polling once settled), shows the job's error message on failure, disables the button while a refresh is active. `UsersManager`: lists every user with their stats, shows an error when the list fails to load, expands/collapses a user's recipe list on toggle, paginates it 10-per-page independently per row, shows a message for a user with no recipes. `AccountPage`: shows the logged-in user's email once loaded, shows an error if the profile fails to load, submits a password change and shows the success/error message. `ImportPage`: shows a category selector on an owned recipe preselected to its current category, changing it calls the API and updates the selection, shows an error on failure, links an owned recipe to its edit page, asks for confirmation (the same shared `ConfirmDialog` the backoffice delete uses) before deleting an owned recipe and removes it from the list once confirmed, does nothing if the confirmation is dismissed, shows an error if the delete call fails, submitting a fresh import via the embedded `ImportManager` form makes the resulting recipe appear once created, with no navigation/reload (the full `ImportManager` → `ImportPage` → `PendingImportsPanel` hand-off, not a single component in isolation). `PendingImportsPanel`: renders nothing with nothing pending, shows a just-finished import's card with a working Edit link, shows the AI-selected category name resolved from the recipe's `category_id`, shows the Instagram play-button warning only for an Instagram `source_url`, acknowledging calls the API and removes the card via the parent callback, shows the right canned message per `error_kind` (including the dedicated `not_a_recipe` message and the `technical`/unset fallback), dismissing a failed job calls the API and removes its card, ignores an already-dismissed job, calls the parent's `onImportsPolled` on every poll tick while a job is still active so a just-finished import's recipe shows up without a page reload, reloads immediately (jobs *and* submissions) when an external `reloadTrigger` bump signals a job was just created or approved elsewhere on the page, and never shows another user's failed job even when the viewer is an admin. `RecipeEditPage`: preloads a recipe's current fields, denies access to a non-owner non-admin, allows an admin editing someone else's recipe, saves and navigates back to the import page, shows an error on a failed save. `FailedImportsManager`: lists a failed job with its full raw error and `error_kind` badge, shows the "Not a recipe" badge for that error kind, omits the category badge entirely for the common case of a job that never reached category resolution (`category_id` null), shows an empty state, Retry/Delete both call the right endpoint (Delete behind the shared confirm dialog), marking as reviewed calls the API and reloads, paginates via the shared `Pagination` component. `ContactPage`: requires the CAPTCHA to be solved when logged out and submits successfully once it is, skips the CAPTCHA entirely when logged in (and never even fetches the Turnstile site key in that case), requires at least an email or a phone number, accepts a phone number alone, rejects a message under 50 characters, shows the server's error message on a failed submission. `SharePage`: shows the teaser (no recipe content) and stashes the pending-share token while logged out, shows the full recipe plus a working "Copy to my recipes" button once authenticated (and navigates to the new recipe's edit page on success), hides that button and shows "This is your own recipe." instead when the authenticated viewer is the recipe's own owner, shows a single message for an expired or revoked link. `ShareRecipeDialog`: generates a link and adds it to the list, copies the link to the clipboard with a "Copied!" confirmation, revokes a link (replacing its actions with a revoked message), closes on Escape. `pendingShare`: plain localStorage round-trip (set/get/clear, a later set overwrites an earlier one). `App.test.tsx` (rendered under `<StrictMode>` specifically — see below): landing on `/login` already authenticated redirects to the pending share page, not home; with no pending share, redirects home as before. The send-icon entry points (`RecipeBrowser`, `RecipeDetail`, `PublicRecipeDetail`): clicking it opens `ShareRecipeDialog` scoped to the clicked recipe, showing the generate/list flow for a private one and the permanent public link (with `listRecipeShares` never called) for a public one. `ShareRecipeDialog`'s own public/private branch: a public recipe shows the permanent `/{lang}/recipes/{id}-{slug}` URL and copy button with no generate button and no share list, a private one gets the original generate/list/revoke flow |
...
- Recipe sharing was verified live end to end against the dev stack after applying migration
  `0037`: as one account, generated a share link for a recipe via `ShareRecipeDialog` and confirmed
  its `expires_at` in the dialog matched the DB row exactly. In a logged-out session, opened the
  link and confirmed the teaser (thumbnail/title/description) showed without the sender's email.
  Signed up manually with a fresh account, verified the real email-verification token (fetched from
  `email_verification_tokens` via `psql`, standing in for clicking the emailed link), and logged in
  — confirmed landing back on the share page, not the home page, completing the localStorage-based
  `cookbook-pending-share` hand-off through the full 3-page signup→verify→login chain. This live
  run is what caught a real bug, since it exercises React's actual `<StrictMode>` double-effect
  behavior (dev-only, mount→cleanup→mount) that a plain unit render doesn't by default: the
  redirect's first version cleared the pending token and navigated inside an `App()`-level
  `useEffect`, racing the `/login` route's own `<Navigate to="/">` — and losing that race on
  StrictMode's second effect invocation, which re-read the token *after* the first invocation had
  already cleared it, saw nothing, and silently redirected home instead. Fixed by moving the
  decision into the route itself (`PostAuthRedirect`, replacing the plain `<Navigate>`) and reading
  the pending token exactly once via a lazy `useState` initializer rather than inside the effect —
  see [Design Decisions](#design-decisions), "Recipe sharing", and the regression test in
  `App.test.tsx` that renders under `<StrictMode>` specifically to keep catching this class of bug.
  Clicking "Copy to my recipes" then landed on `/recipes/{id}/edit` with every field, all 22
  images, and category populated; confirmed in Postgres that the copy was an independent row
  (`shared_from_recipe_id` set, distinct `owner_user_id`) and that deleting the *original* recipe
  afterward left the copy's (physically-duplicated) image files intact.
- Recipe search was verified live against the dev stack after applying migration `0036`
  (confirmed `pg_trgm`, `unaccent`, the `f_unaccent` function, and
  `ix_recipe_translations_title_trgm` via `\dx`/`\df f_unaccent`/`\d recipe_translations`):
  on the front-office recipes page, typing a partial title narrowed the grid ~1.5s after typing
  stopped; on the backoffice Recipes page, a two-word query typed in reverse order ("vinete
  tocanita") still matched "Tocanita simpla de vinete...", confirming the any-order AND matching
  against the real Postgres index, and typing "briose" (no diacritics) matched "Brioșe cu
  dovleac..." against the real `f_unaccent`-backed index; on the Import page, a search narrowed
  the already-loaded "My recipes" list client-side with no network request. A non-matching query
  showed the new "no results" text in place of the grid/list on all three pages.
...
- AI-suggested import categories were verified live end to end: submitted
  `https://savoriurbane.com/clatite-pufoase/` (a real chocolate-pancake recipe) with the category
  picker gone from the "Import from URL" form entirely, approved the resulting `pending` job (this
  account is an admin), and watched it finish `done` with `import_jobs.category_id` resolved to
  the real "Desserts" category — confirmed both via `psql` and in the Account page's "Recently
  imported" card, which showed "Categorie aleasă de AI: Desserts" with no manual page reload.
- The contact page's email delivery was verified live end to end against this deployment's real
  SMTP provider (Yahoo Mail), and that verification is what caught a real bug: the first live
  submission landed in `contact_messages` correctly (`status=queued` → the worker picked it up)
  but the send itself failed with `550, b'Request failed; Mailbox unavailable'`. Isolated the
  cause by hand (`docker compose exec worker python`, building the message directly): a bare
  message with no extra headers sent fine; adding a `Reply-To` — even to another `@yahoo.com`
  address, not just the visitor's own — reproduced the exact same 550 every time. Yahoo's relay
  silently rejects the whole send when `Reply-To` doesn't match the authenticated From/login
  address; this isn't specific to this app in any way, but it meant the "let the owner just hit
  reply" convenience (`worker/app/contact_handlers.py`'s original `_send_contact_email`) was
  actively breaking every single contact-form email in production. Fixed by dropping the
  `Reply-To` header entirely — the visitor's email/phone are already the first lines of the body,
  so replying is still a one-line copy-paste away, and reliable delivery matters far more than
  that one saved click. Re-verified after the fix: the same message resent `DONE` with no error,
  and a second, full round trip through the actual browser UI (`/contact`, phone-only submission,
  logged in — no CAPTCHA needed) landed `contact_messages.status=DONE` end to end via the real
  API → RabbitMQ → worker path, not just a direct handler call.
...
- Username removal and the forgot/reset-password flow were verified end to end against the real
  dev stack, not just unit tests: migration `0033` applied cleanly against the real dev Postgres
  (`\d users` confirmed `username` gone and `email` `NOT NULL`, `\d password_reset_tokens` showed
  the new table), and logging in as the real admin account by email (`POST /auth/login` via
  `curl`, then again through the actual browser UI) succeeded with the real password. In the
  browser: the Account page shows only Email (no Username row); the Users page's table and
  per-user "Show recipes" expansion (now keyed by id, not username) both render against the three
  real accounts in this dev database; the admin Recipes list's "owned by {email}" badge renders
  correctly. Signing up with the already-registered admin email showed the inline "already
  registered" message with a working "Forgot your password?" link; following it to
  `/forgot-password`, solving the real Cloudflare Turnstile widget, and submitting produced the
  generic success response and a real `password_reset_tokens` row plus a `DONE` `email_jobs` row
  (confirmed via `psql`, using this environment's already-configured dev SMTP) — the real admin's
  password was deliberately **not** actually reset through this flow, to avoid changing a live
  credential as a side effect of verification; `/reset-password` with an invalid token was
  confirmed separately to show the correct inline error instead.
- Both the per-category portion weight and the `recipe_ingredient_links` uniqueness fix were
  diagnosed and verified against real recipes, not fixtures. For the servings bug: summed recipe
  61's actual `recipe_ingredient_links.estimated_grams` in Postgres (1170g total) to confirm the
  math behind its wrong "2 servings," then after the fix re-ran `handle_nutrition_job` for real
  (`docker compose exec worker python`) and confirmed both in `psql` (`estimated_servings = 11`)
  and in the browser (recipe page showing "Serves ~11 · ~1.2 kg total" at 154 kcal/serving). Then
  swept every existing Desserts-category recipe through the same re-run as a one-time backfill,
  which is what surfaced the second bug: recipe 21 ("Beetroot Hummus") failed with a real Postgres
  `UniqueViolation` on `(recipe_id, ingredient_index)`, traced to its "sare, piper negru" line
  correctly parsing into two separate nutrition items. Applied migration `0035` against the real
  dev Postgres (confirmed via `\d recipe_ingredient_links` that the unique constraint is gone and
  replaced by a plain index), then re-ran that recipe's nutrition job and confirmed via
  `GET /recipes/21/nutrition` that both `salt` and `black pepper` now appear as separate entries
  sharing `"index": 4`, with the job settling on `status: done` instead of failing.
- The Import/Account split and the Favorites filter were verified live against the real dev stack
  as a logged-in user (a temporary verified account created directly in Postgres, its bcrypt hash
  matching a real `POST /auth/login`, token injected into `localStorage` under the app's own
  `cookbook-auth-token` key — not a mock). Confirmed in the browser: the header nav shows both
  **Import** and **My account**; `/account` renders only the email + change-password section, no
  favorites or recipe list; `/import` renders the "Add or import recipes" heading, "Add a recipe"
  link, URL/bookmark import forms, and "My recipes" list; the recipes page shows an "Add or import
  recipes" button (linking to `/import`) and a rose-colored Favorites pill after the category
  pills, both absent when logged out. Favoriting a recipe via its card, then toggling Favorites on,
  replaced the My recipes/From the community split with a flat "Your favorites" list containing
  only that recipe; selecting a category (Mains) while Favorites stayed on correctly kept showing
  the same recipe (it's in Mains) — combinability confirmed, not just implemented. The temporary
  account and its favorite were deleted from Postgres afterward.
...
- **Approval gate moved to the import request, not the imported result** — the
  ...
  **Approve**; a *successful* import then publishes the resulting recipe immediately
  ...
  review of the same URL's output mostly adds friction without much new signal. Bookmark import
  (see "Bookmark import" below) turned out not to need revisiting this after all: the concern here
  was specifically about *AI-discovered* URLs landing unreviewed in bulk, but bookmark import never
  has Claude/DeepSeek pick which links to import — the user explicitly checks each one from a plain
  list of their own bookmarks, same trust level as typing a URL by hand, just several at once. An
  admin's own bookmark-derived jobs also still land `pending` one at a time, same as any other
  admin import, so nothing about this tradeoff actually changed.
  **Update:** the API-level gate itself is unchanged (an admin's job still lands `pending`, still
  isn't published until `POST /imports/{id}/approve`), but `ImportManager.handleSubmit` (the
  single-URL form on the account/back-office import page) now calls that approve endpoint itself,
  immediately, the moment `createImportJob` returns a `pending` job — so submitting a URL there
  starts the fetch/Claude/image pipeline right away instead of leaving a second **Approve** click
  sitting in front of it. The rationale above hasn't changed, it just used to stop one step short of
  it: an admin typing/pasting a URL into this form and clicking **Add** has already made the same
  call an **Approve** click used to confirm, so the extra click was friction, not an independent
  safety check. The manual **Run import** button stays in the UI for the case the auto-approve call
  itself fails (the job is left visibly `pending`, same as before this change) and for **Retry** on
  a `failed` job, which stays a deliberate manual action. Bookmark import's own admin jobs are
  intentionally **not** included here (`BookmarkImportPanel` still leaves them `pending`) — that
  path can create several jobs from one submission, and collapsing "select these links" with
  "and start fetching all of them" is a bigger jump than the single-URL case.
- **Duplicate URL prevention, scoped per owner** — `POST /imports` rejects (`409`) a `source` that
  ...
  imported" error for a recipe they can't even see. This is still exact string matching, against
  whatever `source`/`source_url` is actually stored — see "Instagram URL normalization" below for
  the one host where "exact string" turned out not to be good enough in practice. Deleting a job
  frees its URL up for resubmission for that user, same as if it never existed.
- **Instagram URL normalization.** A real prod report: a user (verified via `import_jobs`/`recipes`
  on the live database) had imported the same Instagram reel three separate times, each landing as
  its own recipe, with no rejection at any point — the opposite of the "duplicate" error he
  actually reported hitting elsewhere. Root cause: Instagram appends a per-share tracking token
  (`?igsi=...`) to a post's URL that's different every time the link is re-shared or re-copied from
  the app, and the duplicate check above is an exact string match — so re-importing "the same" link
  looked like a brand-new URL every time. (The three stored URLs for that user's case were
  literally the same token truncated by different amounts — `igsi=MTRtY2`, `igsi=MTRtY2x`,
  `igsi=MTRtY2xsZ3RhMTB2NA==` — pointing at a client-side paste/copy issue as the proximate trigger,
  but the real bug is that the app had no way to recognize any of the three as the same URL at
  all.) Fixed with `app/url_normalize.py`'s `normalize_source_url`: for `instagram.com`/
  `www.instagram.com`/`instagr.am` hosts only, the query string and fragment are dropped and a
  trailing slash enforced, before the URL is compared *or* stored — every other host is left
  untouched, since a query string can be genuinely content-identifying on an arbitrary recipe site
  (unlike Instagram, where the path segment alone is what's actually fetched). Applied at every
  entry point that creates or checks a `source`: `POST /imports`, `POST /imports/bookmark/parse`'s
  `already_imported` flag, and `POST /imports/bookmark` — the last of which also needed a second,
  normalized-value dedup pass *within* one bookmark batch, since two raw share-link variants of the
  same reel selected together would otherwise still create two jobs. Existing rows were backfilled
  by migration `0038` (row-by-row in Python, mirroring `url_normalize.py`'s logic rather than
  importing it, so a later change to that module can't retroactively change what an already-applied
  migration did) — a pure data cleanup, not a schema change, and deliberately *not* extended to
  merging/deleting any resulting duplicate recipes (that's a judgment call about which copy to keep,
  left to a human, not something to script blindly across every affected user).
...
- **Ingredient nutrition** — recipes rarely give exact weights ("1 onion," "a pinch of salt"), and
  ...
  <br><br>
  A real recipe (id 21, "Beetroot Hummus") surfaced a second bug: `recipe_ingredient_links` used to
  be unique on `(recipe_id, ingredient_index)`, on the assumption that one source line maps to
  exactly one food. A line like "sare, piper negru" ("salt, black pepper") breaks that assumption —
  Claude's `parse_ingredients_for_nutrition` correctly returns two separate items (salt and black
  pepper have very different nutrient profiles) both pointing at the same `line_index`, since they
  really do share one source line. The second insert then violated the unique constraint, failing
  the whole nutrition job for that recipe with no partial data saved. Fixed by dropping the
  uniqueness (migration `0035`) down to a plain, non-unique index — `ingredient_index` still ties a
  link back to its source line and still drives `ORDER BY` in `GET /recipes/{id}/nutrition`, it just
  no longer has to be one row per line. The alternative (teaching Claude to merge multi-food lines
  into a single item) was rejected: it would have thrown away real per-food nutrition precision to
  satisfy a constraint that didn't need to exist.
- **Estimated servings: computed from total weight, not a separate Claude guess — found by a user
  ...
  <br><br>
  The "one constant for every recipe" call above didn't hold up: a user debugging a pumpkin-muffin
  recipe (id 61) found its 1170g of batter-plus-frosting came back as "2 servings" at ~585g each —
  `STANDARD_PORTION_GRAMS` is a *main-course* portion, and a dessert's real portion (one muffin, one
  cake slice) is a fraction of that. `_PORTION_GRAMS_BY_CATEGORY` in `nutrition_handlers.py` now
  overrides the default for specific categories (Desserts: 110g, Supe: 350g — between the FDA's
  245g soup RACC and a full main course), matched case-insensitively against the recipe's actual
  category name the same way `handlers._resolve_category_id` matches Claude's suggested import
  category. This *is* the per-category table the original decision explicitly rejected — revisited
  because "categories are free-text, so no table could be reliable" turned out to be less costly in
  practice than "every dessert's serving count is wrong": any category name not in the table (a
  typo, an admin-renamed category, one of the free-text categories the original reasoning worried
  about) just falls back to `STANDARD_PORTION_GRAMS`, so an unrecognized name degrades to the old
  behavior rather than erroring or silently misfiring. Verified live: recipe 61 went from "2
  servings" (~650 kcal each) to "~11" (154 kcal each) after re-running its nutrition job, matching
  the visible size of a batch of muffins; a one-time backfill (re-running the nutrition job for
  every existing Desserts-category recipe) then corrected the same under-counting on 17 more
  pre-existing recipes.
- **AI-suggested import category, replacing manual pre-selection.** The "Import from URL" form
  used to require picking a category *before* submitting — awkward, since the requester hasn't
  seen the recipe's content yet and has to guess. Replaced with: Claude picks one from the app's
  real, current category list as part of the same extraction call that already returns
  translations/images, and the worker resolves that free-text answer to a `category_id` after the
  fact (`_resolve_category_id` in `handlers.py`). The key constraint shaping the design: categories
  are free-text, admin-created rows (`CategoryManager`), not a fixed enum — so there's no static
  schema Claude could be trained against. Instead the current list is sent as literal text in the
  prompt each call (`_category_instruction` in `claude_client.py`), and Claude is told to return
  one of those exact names verbatim. Nothing stops a returned name from being a near-miss (typo,
  case difference) or stale (a category deleted mid-flight) — `_resolve_category_id` matches
  case-insensitively and, on no match at all, falls back to the first category by id rather than
  failing the whole import: a wrong category is a one-click fix on the owner's own Edit page, not
  worth losing an otherwise-successful import over. `import_jobs.category_id` had to become
  nullable (migration `0030`) to represent "not resolved yet" for a job still mid-flight or one
  that failed before reaching that point — which is also why `FailedImportsManager` has to guard
  against a null `category_id` for display: most failures never get that far, since category
  resolution happens near the end of the worker's try block, after fetch/extraction already
  succeeded. `reparse_handlers.py` shares the same extraction functions and so still has to build a
  category list for the prompt, but deliberately ignores the returned suggestion — a reparse
  re-scrapes an *existing* recipe whose category the owner may have already corrected, and silently
  overwriting it on every content refresh would be a surprising side effect of what's meant to be a
  text-quality fix, not a recategorization pass.
- **Contact page: async job like everything else, optional-auth CAPTCHA gate, and a
  Reply-To header that turned out to actively break delivery.** Three real decisions bundled into
  one feature:
  <br><br>
  *Async, not synchronous.* Every other outbound-email flow in this app (`email_jobs` for account
  verification) goes through the RabbitMQ → worker path rather than calling SMTP inline from the
  API request, so the contact form does too — a new `contact_messages` table/`contact_message_jobs`
  queue, not a special-cased synchronous `smtplib` call bolted onto `routers/contact.py`. This
  keeps a slow/flaky SMTP server from ever blocking the form's response, and means a broken queue
  publish degrades the same way it does everywhere else (recorded on the row as `failed`, not
  surfaced as an error to the visitor — see `_queue_and_publish`'s equivalent in `routers/imports.py`
  for the established pattern this mirrors).
  <br><br>
  *Optional auth, not two separate endpoints.* The brief called for the form to work both logged in
  and out, with a CAPTCHA only for the logged-out case. Rather than a second route or a query-param
  branch, `routers/contact.py` takes `current_user: AuthUser | None = Depends(get_optional_current_user)`
  (already used elsewhere for public recipe browsing) and only requires/verifies a Turnstile token
  when `current_user is None`. The frontend mirrors this: `ContactPage` only fetches the Turnstile
  site key and renders the widget when `!isAuthenticated`, so a logged-in visitor's form has one
  fewer network call and no CAPTCHA to solve at all, not just a hidden-but-still-required one.
  <br><br>
  *Email-or-phone, enforced once, not twice.* "At least one of email/phone" is a cross-field rule
  that doesn't fit a single `Field(...)` constraint — implemented as a Pydantic `model_validator`
  on `ContactMessageCreate` (the real, authoritative check) and mirrored client-side in `ContactPage`
  purely for a faster inline error; the server-side version is what actually matters; a corner case
  is `submitted_by_user_id`, which needed a nullable FK with `ON DELETE SET NULL` (not `CASCADE`,
  unlike most of this app's other user-owned rows) — a contact message is a record of an event, not
  user-owned content, so deleting the account later shouldn't delete the message, just anonymize it.
  <br><br>
  *The Reply-To bug, found only by testing against the real SMTP provider.* The first version set
  `Reply-To` to the visitor's own email, so the site owner could just hit reply. Live-tested against
  this deployment's actual Yahoo Mail SMTP account and every single send failed — `550, b'Request
  failed; Mailbox unavailable'` — with no obvious cause from the error text alone. Isolated by hand
  (`docker compose exec worker python`, building the message directly, one header at a time): a bare
  message sent fine; adding *any* `Reply-To` that didn't match the authenticated From/login address
  (tested with both the visitor's address and an unrelated `@yahoo.com` one) reproduced the exact
  550 every time — an anti-spoofing policy on Yahoo's relay, not a mistake in this app's code, but
  one that would have silently broken every contact-form email in production if the "convenience"
  header had shipped unverified. Fixed by dropping `Reply-To` entirely; the visitor's email/phone
  are already the first lines of the body, so replying is still one copy-paste away — reliable
  delivery is worth more than the one click saved. See Testing's "Notes on the current setup" for
  the full verification trail (`test_handle_contact_message_job_...` in `test_contact_handlers.py`
  now asserts `Reply-To` is *never* set, which is itself a record of the bug this fix avoids
  reintroducing).
...
- **Import visibility** — the back office shows each request's status, backed by `import_jobs.status`,
  polling every 3s while any job is `queued`/`fetching`/`processing`. Bookmark import follows the
  exact same `import_jobs`-per-source pattern as a single URL, one row per selected link, so this
  same status/polling machinery covers it for free — see "Bookmark import" below. A pasted list of
  several URLs in the single-URL form itself is still the one gap left.
...
- **DeepSeek as an alternate AI provider** — motivated purely by cost: DeepSeek's list pricing
  (`deepseek-v4-flash`/`deepseek-v4-pro`) runs roughly 7-18x cheaper per token than the
  Haiku/Sonnet pair this app already uses, and the ask was to have both available in parallel with
  an admin toggle, not a wholesale replacement — accuracy on this app's actual messy recipe-page
  HTML hadn't been verified yet, so keeping Claude as the default and DeepSeek as an opt-in was the
  only responsible way to ship it. The integration turned out unusually cheap to build: DeepSeek
  publishes its own Anthropic-API-compatible endpoint
  (`https://api.deepseek.com/anthropic`) that speaks the same Messages API wire format as
  `api.anthropic.com`, including forced `tool_choice` (`{"type": "tool", "name": ...}`), which is
  exactly what all four `claude_client.py` functions depend on for structured, schema-shaped
  output. That meant no second SDK, no parallel request-building code path, and no changes at all
  to the four tool-schema dicts — `claude_client._client(provider, api_key)` just constructs the
  same `anthropic.Anthropic` client with a different `base_url` when `provider == "deepseek"`, and
  a small `_PROVIDER_MODELS` table maps each provider onto its own cheap/quality-tier pair
  (`deepseek-v4-flash` for the three extraction-style calls, `deepseek-v4-pro` for
  `translate_recipe` — mirroring the existing Haiku/Sonnet split, see "Claude cost optimization"
  above). The provider lives on `app_settings.preferred_ai_provider`, following the exact same
  admin-settings extension pattern as every other setting (model → schema → router →
  `settings_service` on the API side, mirrored into the worker's copies) — an admin flips it and
  the very next job picks it up, no restart, same as any other setting (see "Admin-editable
  settings" above). `SettingsSnapshot.ai_api_key` resolves which literal key
  (`anthropic_api_key` vs `deepseek_api_key`) a call should use from that one flag, so every
  handler call site passes `(app_settings.preferred_ai_provider, app_settings.ai_api_key)` without
  re-deriving the mapping itself. Two things this design deliberately does *not* do: it never
  relies on DeepSeek's own Claude-model-name auto-mapping (sending `model="claude-opus-5"` and
  letting DeepSeek's endpoint silently substitute its own model) — explicit DeepSeek model IDs are
  used instead, so the model actually in use is never implicit; and it doesn't touch
  `extract_recipe`'s image handling, since the app only ever sends plain text/HTML to the model,
  never image content blocks — one of the compat endpoint's real limitations (no vision/document
  content types) that turned out not to matter here. Not yet done: a real accuracy comparison
  against a handful of live recipe imports — the toggle exists so that can happen without a code
  change, not because DeepSeek's extraction quality on this app's specific messy-HTML input has
  been validated. The first live test against DeepSeek immediately surfaced a real bug: importing
  [chefi.ro's dark-chocolate recipe](https://chefi.ro/reteta-de-ciocolata-neagra-de-casa-fara-zahar-de-ce-ingrediente-ai-nevoie-pentru-un-desert-perfect-80158.html)
  failed with `400 Thinking mode does not support this tool_choice`. DeepSeek's compat endpoint
  runs `deepseek-v4-flash`/`-pro` in thinking mode by default, and unlike real Anthropic (where the
  same forced-`tool_choice` calls have always worked without ever setting a `thinking` param at
  all), DeepSeek's implementation rejects forced tool_choice outright whenever thinking is active —
  every function in `claude_client.py` needs forced tool_choice for guaranteed structured output,
  so the two are fundamentally incompatible for this app's usage. Fixed by a small
  `_thinking_kwargs(provider)` helper that adds `thinking={"type": "disabled"}` to the request only
  when `provider == "deepseek"` — deliberately *not* applied to Claude calls, since they've never
  needed it and there was no reason to risk changing working behavior to fix a DeepSeek-only issue.
  With that fixed, the accuracy comparison actually happened: reparsing 4 of an existing user's
  real imported recipes (lauraadamache.ro, savoriurbane.com, jurnalderetete.md,
  lauralaurentiu.ro) through DeepSeek and checking each result against its live source page in a
  browser, not just diffing against the old Claude version. Two findings, one good and one bad.
  The good one: DeepSeek's factual accuracy held up — on one recipe (savoriurbane.com's basic
  pancake recipe) it was actually *more* accurate than Claude's original extraction, which turned
  out to have silently hallucinated a generic sugar-and-oil batter with no relationship to the real
  page's butter-and-mineral-water recipe, a pre-existing Claude bug nobody had noticed until this
  comparison surfaced it. The bad one: DeepSeek was measurably worse than Claude at following
  `_SECTION_HEADER_INSTRUCTION` — on 2 of 4 recipes it turned a source page's own generic
  "Ingredients"/"Method" heading (labeling the *entire* list, not a real sub-recipe) into a
  `SECTION_HEADER_PREFIX`-marked array entry, and it copied the source page's own literal step
  numbers ("1. ...", "2. ...") straight into step text — a real, visible bug, since the frontend
  numbers steps itself, so a recipe would render "1. 1. Scurgeți...". Fixed by tightening
  `_SECTION_HEADER_INSTRUCTION` with two explicit prohibitions (never mistake a whole-list label
  for a sub-group marker; never carry over the source's own step numbers) rather than adding
  DeepSeek-specific prompt branching — the instruction is shared with Claude's extraction calls,
  and there's no reason Claude couldn't make the same mistake on a differently-formatted page.
  Re-running the same 4 reparses against the tightened prompt: the step-double-numbering bug was
  completely gone, and 3 of 4 recipes came back with no spurious headers at all — including one
  recipe where the fix additionally *restored* an itemized ingredient sub-list (croutons on the
  soup recipe) that DeepSeek had dropped on the first pass, now correctly split under its own
  `### Crutoane` marker on both ingredients and steps. One recipe (the hummus one) still added a
  single residual whole-list-style header on its steps array despite the tightened wording — a
  minor, cosmetic-only imperfection (no content loss, no double-numbering) rather than the
  double-numbering bug the fix specifically targeted, and not yet worth a second prompt-tuning
  pass given the size of the improvement already measured.
- **Bookmark import: parse-then-select, not parse-then-auto-queue** — the ask was specifically a
  two-step flow (see the request that shaped this: upload a bookmark file, see the candidate
  links, pick which ones, *then* import), which is also what made the quota interaction sane:
  `POST /imports/bookmark/parse` is read-only (no `ImportJob` rows, nothing written) and just
  returns candidate links plus the caller's `remaining_quota`, so a user can look before committing
  any of their limited quota to links they don't actually want. Selection is capped client-side at
  `remaining_quota`, but the server re-enforces the same cap on `POST /imports/bookmark` rather
  than trusting the frontend — the real limit-check authority is the same
  `max_imports_per_user`/`imported_recipes_count` pair the single-URL path already uses (see
  "Lifetime import limit" below), just applied to a batch: `_remaining_quota` is
  `_ensure_import_limit_not_reached`'s counterpart, returning `None` for an admin (still exempt)
  instead of raising. A link already imported or already queued is filtered out **before** the
  quota check, not after — otherwise reselecting something you already have would needlessly cost
  a slot of a limit you're not actually about to exceed — and reported back as `skipped_duplicate`
  rather than failing the whole batch over one stale entry (parse-time and confirm-time can drift
  if the user takes a while to pick). Deliberately not built: any AI involvement in bookmark
  parsing itself — `bookmark_parser.py` is a plain BeautifulSoup `<a href>` scan, no Claude/DeepSeek
  call, because the value here is exposing the user's *own* bookmarks for them to choose from, not
  a model guessing which ones are "really" recipes (and see the "Approval gate" entry above for why
  that distinction also mattered for the admin-approval tradeoff). Each selected link becomes an
  ordinary `import_jobs` row (`type=bookmark`, otherwise identical to `type=single`) that runs
  through the exact same worker pipeline — `worker/app/handlers.py`'s only type branch is
  `INSTAGRAM` vs. everything else, so `bookmark` needed zero worker changes to "just work." The one
  real gap this surfaced: `ImportManager`'s own "in progress" list only ever reloaded from its own
  actions (submit/approve/delete) or its own poll loop, and that poll loop only starts once it
  already has an active job in state — so jobs created entirely through `BookmarkImportPanel`
  wouldn't have appeared there until some unrelated click happened to trigger a reload. Fixed by
  giving `ImportManager` the same `reloadTrigger` prop `PendingImportsPanel` already had for the
  identical reason, both now fed from `AccountPage`'s one shared `importTrigger` counter — bumped
  by either component's own `onJobCreated`.
...
- **Incident: the HTML-cleaning cost fix silently deleted lazy-loaded images** — reported live: a
  ...
  the result.
- **A real import that turned out to be three separate bugs, found in three passes.** Reported
  live, twice: first, `mancaregatita.ro/blogs/blog/supa-de-rosii` failed with `not_a_recipe` even
  though the page opens and shows a full recipe in a browser; then, after that got fixed, the
  re-imported recipe had no photo. Same underlying `fetch_page`/`_clean_html` code, three distinct
  root causes.
  1. *First theory (wrong, but the fix stuck anyway): JS-rendered content.* Re-running `_clean_html`
     against the real downloaded page showed `<main>` as an essentially empty shell (~1.5KB of
     nav/header chrome, none of it the recipe) — read at the time as "the theme renders the article
     via JavaScript after load, so a plain `httpx` fetch never sees it." Real or not, the page's SEO
     plugin ("AVADA SEO Suite") had already embedded the complete article text — ingredients, numbered
     steps, tips — in a JSON-LD `Article` block's `articleBody`, written for search engines rather
     than a browser DOM. `_extract_json_ld_article_bodies` recovers that text (checked against
     `Article`/`BlogPosting`/`NewsArticle`, the common types such a plugin emits) *before* `<script>`
     tags are stripped as noise, picking the longest match when a page embeds more than one
     near-duplicate block. This fixed the immediate `not_a_recipe` failure and is real, useful
     defense-in-depth for pages that genuinely are JS-only — but it turned out not to be *this*
     page's actual problem.
  2. *Real root cause, found while chasing the missing photo: a stdlib `html.parser` corruption
     bug, not JS rendering at all.* The photo was in the raw HTML the whole time. Parsing the same
     downloaded page with `lxml` instead of `html.parser` recovered the real article body directly
     from the DOM — 5,969 characters of real content where `html.parser` had produced ~1.5KB of
     shell. Traced to a single malformed `<link rel="stylesheet" media="print" onload="...">` deep
     in the page's `<head>` (no `data-*`/noise-attribute issue, no self-closing-slash issue — a
     narrower repro with just that tag in isolation didn't even reproduce it, only the full,
     real 320KB document did) that desynced `html.parser`'s open-tag stack badly enough that
     everything parsed after it — the entire real article, image included — nested inside spurious
     tags that `_clean_html`'s own noise-stripping then `decompose()`'d, along with the corrupted
     wrapper. `html.parser` is stdlib's own documented weak point against real-world malformed
     markup; `lxml` (libxml2-backed) is what BeautifulSoup's own docs recommend for exactly this.
     Switched `BeautifulSoup(html, "html.parser")` to `BeautifulSoup(html, "lxml")` — the one
     other place `_clean_html`'s output shape actually depends on the parser (bare content with no
     explicit `<body>` now gets one synthesized, since lxml follows the HTML5 spec here and
     `html.parser` didn't) needed three test assertions updated to match, not a behavior fix.
  3. *A third, independent bug, only visible once the image tag was reachable at all: a
     "download succeeded" image that's actually a 1×1 placeholder pixel.* The recovered `<img>`'s
     `data-src` wasn't the real photo URL — it was a *template*, pre-filled with a tiny placeholder
     width (`...webp?v=...&width=1`, a "load almost nothing yet" lazy-load trick), with the real
     candidate widths listed in a sibling `data-widths="[100,200,...,1600]"` attribute that the
     page's own JS reads from to pick a size at render time. `_promote_lazy_image_sources` was
     promoting that placeholder URL as-is — a technically successful download (real HTTP 200, real
     JPEG bytes) that `images.process_images`'s failed-download skip could never catch, since
     nothing about it looked like a failure. `_widen_templated_lazy_url` now detects a `width=`
     query param alongside a `data-widths` list and substitutes in the largest available width
     before promotion — the same value the real page would have settled on. A normal (non-
     templated) lazy `src` is left untouched.
  All three verified against the real page and the real pipeline, not synthetic fixtures, across
  both passes: re-cleaned the actual downloaded HTML each time to confirm the fix before touching
  the pipeline, then re-ran the real `handle_import_job` against the real recipe end to end —
  landed with a correct, fully-populated recipe (10 ingredients, 14 steps, 3 tips, both languages)
  and, after fix 3, a real 600×600 JPEG instead of a 286-byte placeholder — confirmed live in the
  browser both times.
...
- **Local backup strategy: Postgres + images only, oldest-survivor-in-range retention, and no
  offsite destination baked in.** Scoped deliberately narrow rather than reaching for a generic
  backup tool: the only durable state this app owns lives in two places — Postgres (everything
  structured: recipes, users, jobs, settings, ...) and the `images_data` volume (recipe photos).
  `rabbitmq_data` was explicitly left out — it only ever holds in-flight job messages, which are
  either already reflected in Postgres (a job's own `status`/`error` row) or safely re-enqueueable;
  backing it up would just be backing up noise. The script deliberately does **not** push anywhere
  off the VPS — the operator already has (or will set up) their own pull mechanism on their own
  schedule, and baking in a specific offsite target (S3 bucket, rclone remote, ...) would mean a
  credential this repo has to manage for a destination that's really the operator's choice, not
  this app's concern.
  <br><br>
  Two real bugs surfaced from testing the script against the actual dev stack rather than just
  reading it — both only found because the deployed version was actually exercised, not just
  code-reviewed. First: the initial draft did `source .env` on the host to get
  `POSTGRES_USER`/`POSTGRES_DB`, which broke immediately — `.env` has unquoted values containing
  spaces and parentheses (`SCRAPE_USER_AGENT=Mozilla/5.0 (X11; Linux x86_64) ...`), which isn't
  valid shell syntax to `source` directly, only to load via `docker compose`'s own `env_file:`
  parser (far more lenient). Fixed by not touching `.env` on the host at all — `docker compose
  exec -T db sh -c 'pg_dump -U "$POSTGRES_USER" -d "$POSTGRES_DB"'` reads those two vars from the
  `db` container's own environment instead, already populated from `.env` by docker-compose.yml
  when the stack started. The images side follows the same "reuse what's already running"
  approach: `docker compose exec -T api tar czf - -C /data/images .` streams the archive straight
  out of the `api` container, which already has the volume mounted at that exact path — no
  throwaway container, and no need to know the volume's actual Docker-assigned name.
  <br><br>
  Second, and more fundamental: the original retention rule deleted any backup whose age (in
  days) wasn't *exactly* 0, 1, 4, or 8, re-evaluated fresh on every run. That rule was unit-tested
  against a synthetic set of files pre-created at every age from 0 to 10 and looked correct — but
  that test only proves the rule picks the right files out of a *complete* history handed to it
  all at once. It doesn't reflect how the script actually runs: once a day, incrementally, on
  whatever survived the *previous* day's prune. Simulating 10 consecutive daily runs (create
  today's backup, then prune, repeat) showed the real behavior: a file that's 1 day old is kept,
  but the moment it turns 2 days old — the very next run — it's deleted, because 2 isn't in
  `{0,1,4,8}`. Nothing ever gets the chance to survive from 2 days old up to 4, so the "~4 day"
  and "~8 day" backups the retention was supposed to produce never actually existed; only the
  last 2 daily backups ever did. This surfaced not from a code review but from a user asking "how
  does this work" after the script had already been running in production for a day — a question
  worth taking literally enough to trace through, rather than just re-describing the intent.
  <br><br>
  Fixed by keeping the single **oldest** survivor within each target range (age 2–5 for the
  "~4 days ago" slot, age 6–9 for "~8 days ago") instead of matching an exact age. This is what
  makes a backup able to *persist* through the gap ages: once a file claims a range by being the
  oldest thing in it, it keeps winning against younger challengers (which get deleted immediately,
  since they lose the "oldest in range" comparison) every day until it ages out of the range
  entirely — at which point the range goes briefly empty and gets re-seeded by whatever next
  reaches its lower bound. Picking the *newest* survivor, or the one *closest* to the target age,
  were both considered and rejected: newest-in-range reduces to the same "instant eviction" bug
  the exact-match rule had, and closest-to-target has its own failure mode — once a backup becomes
  the incumbent, every younger challenger is by definition *further* from the target age than an
  aging incumbent that already passed through it, so the incumbent would win forever and just keep
  drifting older with no eventual rotation. Verified by simulating 25 consecutive daily runs
  against the actual functions extracted from the deployed script (not a hand-copied
  approximation): the kept set stabilizes at 3–4 files, both older slots rotate to a fresh backup
  every few days instead of freezing on one, and nothing is ever kept past 9 days old.
- **Verification email personalized in the signup language, not live-synced afterward** — the
  ...
  `supported_languages`), and the worker sends the email in that language, with a short "what this
  site is" paragraph. Deliberately **not** kept in sync with the user's browsing language
  afterward (switching the site language in the header doesn't rewrite `users.language`) — there's
  no ongoing relationship between "current UI language" and "language of an email" worth
  maintaining, since an account's emails are all one-off transactional messages (verification,
  password reset), not an inbox someone reads over time. Email copy itself only exists for
  `"en"`/`"ro"` in `worker/app/email_handlers.py`, hardcoded independently of `supported_languages`
  — a third configured language would silently fall back to English until someone adds real copy
  for it, the same tradeoff already made for `LANGUAGE_LABELS` and the frontend's own translation
  dictionaries. The email used to greet the user by username ("Hi, {username}"); the prediction
  above ("that would change if a second account email were added") did come true once
  password-reset emails were added — rather than resolve a name for two different email kinds, the
  greeting was simplified to "Hi," / "Salut!" with no name at all, once username stopped existing
  as something to greet anyone by (see "Removing usernames" below).
- **Removing usernames** — the account model originally had both a `username` (used for login and
  as the JWT subject) and an `email` (used only for verification mail), because the login+password
  pattern was built before self-service signup existed. In practice this meant every returning
  visitor had to remember which of two strings to log in with, and the UI never explained the
  distinction — a real point of confusion, not a hypothetical one. Fix: `email` is now the only
  account identifier — `users.username` was dropped entirely (migration `0033`), the JWT `sub`
  claim is the email, and every place that used to look a user up or display them by username
  (recipe ownership self-checks, the admin Users/Recipes/Failed-Imports pages, `GET /recipes`'s
  admin `owner` filter) now uses the numeric `id` for logic and, where a human-readable label is
  actually needed for an admin's own eyes, the email. Two things had to be handled carefully:
  1. *One pre-existing account had no email.* The admin account seeded directly into the database
     by migration `0018` (before self-service signup existed) predates the requirement that every
     signup collect an email, so it had neither. Backfilled by hand (`UPDATE users SET email = ...
     WHERE username = 'danionescu'`) in both dev and prod *before* deploying migration `0033`,
     which also carries a defensive `UPDATE ... WHERE email IS NULL` safety net (synthesizing a
     `username-id@invalid.local` placeholder) so the same situation on some other environment fails
     loudly with an obviously-fake address instead of blocking the migration outright.
  2. *A recipe/import job's owner is shown to more than just its owner* — the admin Recipes list
     shows "submitted by X" for every recipe, and the admin Failed Imports page shows who ran a
     failed import. Exposing a real email address to *every* API caller (including anonymous
     visitors hitting the public `GET /recipes` feed) the way `owner_username` used to be exposed
     would have been a real privacy regression, not a like-for-like rename. Fix: `RecipeRead`/
     `ImportJobRead` now carry both an always-present `owner_user_id`/`created_by_user_id` (just a
     number, harmless to expose, used for the client-side self/ownership checks that
     `owner_username`/`created_by_username` used to power) and an `owner_email`/`created_by_email`
     that the API nulls out unless the *caller* is an admin — reusing the exact nulling pattern
     `ImportJobRead.error` already established for hiding a raw technical string from a non-admin.
     Notably this also nulls a non-admin's own email back to them on their own job — deliberately
     consistent with the `error` precedent rather than carving out a "but it's mine" exception,
     since the point is "only an admin needs to see an email here," not "only strangers shouldn't."
- **Forgot password: same generic response whether or not the email exists** —
  `POST /auth/forgot-password` never distinguishes "sent a reset link" from "no account with that
  email" in its response; both return the identical detail string, and only a real match actually
  gets a token + email. Telling a caller outright that an email isn't registered would turn the
  endpoint into an account-enumeration oracle — trivial to script against a list of candidate
  addresses. The real distinction (did an email actually go out) is something only the account
  owner can observe, by checking their own inbox, which is exactly where it belongs.
- **Sign in with Google: three scoped design choices, made explicitly before building it.**
  1. *A random, unusable `password_hash` instead of a nullable column.* A Google-only account has
     no password to check, but making `users.password_hash` nullable would have meant threading a
     "does this account even have a password?" check through login, `PATCH /users/me/password`,
     and anywhere else that assumes one exists. Instead, account creation (and claiming an
     unverified row) sets `password_hash` to a real bcrypt hash of an unguessable random value —
     every existing password code path stays untouched, since checking a password against it
     simply always fails, exactly like a wrong password would. It also has a nice emergent
     property for free: a Google-only user who later runs "forgot password" gets a real, usable
     password bolted onto their account, with zero extra code — that flow already just overwrites
     `password_hash` unconditionally.
  2. *Proven Google ownership claims an unverified password account.* If someone already started
     (but never confirmed) a password signup with the same email — possibly not even the same
     person — a matching Google sign-in verifies it, links it, and invalidates whatever password
     was on it. This was a deliberate call, not the only reasonable one (the alternative was
     blocking the Google sign-in as a conflict) — but an unconfirmed password account is, by
     definition, unproven; Google having just verified that same mailbox is stronger evidence of
     ownership than a password nobody ever confirmed, and nothing of value could have been stored
     under an account that was never usable in the first place.
  3. *Terms acceptance is deferred to a second request, not blocked up front.* The Google button
     itself stays a genuine one-click action for a *returning* user — the common case — rather
     than gating every click behind an overlay regardless of whether an account already exists.
     Only when `POST /auth/google` determines it's actually about to create a brand-new account
     does it 400 asking for `terms_accepted`; the frontend shows the same `TermsOverlay` component
     manual signup already uses, then resubmits the identical ID token (valid for about an hour,
     comfortably longer than reading the terms takes) with `terms_accepted: true`. Nothing is
     written to the database on the first, terms-less call.
  No client secret is stored anywhere — `google_client_id` (admin-editable, not secret, same
  `GET /settings/public` exposure as `turnstile_site_key`) is all either side needs, since the ID
  token's signature is verified against Google's own published public keys
  (`google.oauth2.id_token.verify_oauth2_token`) rather than a server-to-server code exchange.
- **Blank recipes from a dead source URL.** A real bug report: a stale import URL now 302-redirects
  to an unrelated page (its original post was gone, and the site bounced visitors to the owner's
  bare Instagram profile instead), and the import pipeline dutifully scraped that unrelated page,
  called it a recipe, and saved an empty-but-`APPROVED` recipe with no title, ingredients, or steps.
  Root cause: `claude_client.extract_recipe` calls the Anthropic API with a *forced* `tool_choice`
  (`{"type": "tool", "name": "extracted_recipe"}`) so the response is always a parseable tool call —
  but that also means Claude has no way to say "there's no recipe on this page," so a page with no
  recipe content just comes back as a tool call with every field blank, and nothing downstream had
  ever checked for that. The fix is a content check, not a smarter prompt: both
  `handlers._finish_import` (fresh imports) and `reparse_handlers._update_translations_in_place`
  (workflow #7's re-scrape-in-place) now reject the extraction — via a shared
  `NotARecipeError`/`_is_blank_translation` helper in `worker/app/handlers.py` — when *every*
  returned translation has no title and no ingredients and no steps, rather than trusting a
  structurally-valid-but-empty response. Import failure gets its own `error_kind = not_a_recipe`
  (a new value alongside `disallowed`/`technical`, see [Data Model](#data-model-sketch)) so the
  account and admin UIs can show "this page doesn't seem to contain a recipe" instead of the
  generic "an admin will take a look" message, which would have been actively misleading here since
  no admin action can fix a dead source URL. Reparse has no `error_kind` column to set — it fails
  the job and, more importantly, leaves the existing recipe's translations completely untouched,
  which is arguably the more important half of the fix: without this guard, reparsing a live,
  correct recipe against a since-broken source URL would have silently wiped out real, already-
  published content. A translation with only *some* blank fields (e.g. no tips, or a thin
  ingredient list) is left alone in both paths — that's a legitimately sparse recipe, not a dead
  page, and forced-tool-use still returns real content for whatever the page actually had.
- **Superadmin Users page: three scoped design choices, made explicitly before building it.**
  ...
  2. *The per-user recipe list reuses `GET /recipes`'s existing `owner` filter* (extended to accept
     an explicit user id for an admin, not just `"me"`) *instead of a new endpoint.* The
...
- **Post-import review panel, part 2: an admin's own Account page was leaking every user's
  ...
  `created_by_user_id === user?.id` before rendering (and before deciding whether to poll)
...
- **Splitting Import out of the account page.** A user request, not a bug: the account page had
  grown into three unrelated things stacked in one place — profile/password, a favorites list, and
  the actual work surface (manual add, URL/bookmark import, "my recipes"). Import got its own page
  at `/import`, reached via a header nav link (symmetric with `/account`) and a new "Add or import
  recipes" button on the recipes page itself — the button matters more than the nav link in
  practice, since it puts the entry point where someone already is (browsing) rather than requiring
  a detour through account settings first. `AccountPage` shrank to just profile + password; the
  moved section (`RecipeList`, previously a private helper inside `AccountPage.tsx`) became its own
  component so both the new `ImportPage` and any future consumer can reuse it. Every internal
  "back to account" link that actually meant "back to my recipes" (`RecipeEditPage`'s error/cancel/
  save-redirect, `RecipeSubmitForm`'s post-submit link) now points at `/import` instead — these
  were found by grepping for `"/account"` after the move, not by guessing, since a stale link here
  fails silently (navigates somewhere that still exists, just not where the user came from).
  <br><br>
  Favorites didn't just move — it changed shape, and that took a real design decision, not just a
  cut-and-paste: a list buried in account settings only reflects a moment already decided (you
  favorited it, now here it sits), while the same data as a **filter on the recipes page** stays
  useful while you're actually still browsing — narrow to favorites, then narrow further by
  category, without leaving the page you're already on. That's why it's a toggle pill next to the
  category pills (`CategoryNav`, given its own rose color family so it reads as a different *kind*
  of filter at a glance, not just another category) rather than a page of its own, and why it's
  combinable with a category rather than a third mutually-exclusive option — "favorited desserts"
  is a real, useful question a purely either/or filter couldn't answer. The API grew a matching
  `GET /recipes?favorites_only=true`, reusing every bit of existing pagination/visibility plumbing
  (`_visibility_clause`, the same exclusion applied to `GET /users/me/favorites`) rather than
  inventing a parallel endpoint — `GET /users/me/favorites` itself is unchanged and still exists,
  just unused by the frontend now, kept rather than removed since nothing forces the choice and a
  future admin/API consumer might still want the simple unpaginated form. One real UX consequence
  of turning favorites into a *filter*: it necessarily flattens the recipes page's usual My
  recipes/From-the-community split into one plain list while active, since a favorite can be either
  kind of recipe — reusing that split would have meant showing (often empty) section headers for a
  view that's already been narrowed down by the user's own choice.
- **Recipe structured data: filling in what was free, leaving out what would mean fabricating
  data.** Google Search Console flagged 5 optional fields missing from the `Recipe` JSON-LD
  (`RecipeDetailView.tsx`): `nutrition`, `recipeCuisine`, `cookTime`, `aggregateRating`, `video` —
  all listed as non-critical, meaning the rich-result eligibility itself wasn't at stake, just
  richer search snippets. They split into two very different categories once actually checked
  against what data the app has:
  1. *`nutrition` (and `recipeYield`, not even asked for) needed no new data at all.* Both public
     recipe-detail routes already fetch `GET /recipes/{id}/nutrition` unconditionally for the
     on-page `NutritionPanel`, so `per_serving`/`estimated_servings` were sitting unused right next
     to the JSON-LD `<script>` the whole time. Added as `NutritionInformation` (calories/protein/
     carbohydrate/sugar/fat, matching schema.org's expected `"270 calories"`/`"9 g"` string format)
     and `recipeYield`, both gated on nutrition enrichment actually being `done` — nothing invented
     for a recipe still mid-enrichment or one CalorieNinjas/Claude never resolved.
  2. *`recipeCuisine` and `cookTime`/`prepTime` have no backing data anywhere* — not on the
     `Recipe` model, not in the import-extraction prompt, not on the manual-add form. Adding them
     for real would mean a schema migration, a Claude extraction-prompt change (and every existing
     recipe re-parsed or left blank), and new form fields — a real feature, not a JSON-LD tweak, so
     deliberately left out of this pass rather than guessed at (a category name like "Desserts" is
     a course, not a cuisine — mapping one onto the other would just be a second wrong field).
  3. *`aggregateRating` and `video` were left out on purpose, not as an oversight.* Neither a
     ratings/reviews feature nor recipe video exists anywhere in the app — fabricating either
     (a made-up rating, a non-existent video URL) is exactly the kind of manipulative structured
     data Google's own guidelines penalize, up to losing rich-result eligibility entirely for the
     whole site. These stay absent until the underlying feature (real user ratings, real hosted
     video) actually exists to back them.
  Verified live against a real enriched recipe (`docker compose`'s dev stack, not a fixture):
  loaded `/en/recipes/31-sweet-potato-cakes` in the browser and read the rendered `<script
  type="application/ld+json">` back out via `document.querySelector`, confirming `recipeYield: "4"`
  and a full `NutritionInformation` block matching `GET /recipes/31/nutrition`'s real
  `per_serving` figures.
- **Letting a regular user delete their own recipe.** `DELETE /recipes/{id}` was the one recipe
  mutation still gated `require_admin` outright, rather than the owner-or-admin pattern every other
  one already follows (`PATCH .../category`, `PATCH .../share`, `PUT /recipes/{id}`, `POST
  .../acknowledge-import`) — not a deliberate restriction, just the one endpoint nobody had gotten
  around to relaxing once recipe ownership became a real per-user concept. A user asked for it
  directly: a delete button next to Edit on the Import page's "My recipes" list, behind the same
  confirmation flow the backoffice already uses. Reused rather than rebuilt: `useConfirm`'s
  `ConfirmDialog` (already an "OK, this is destructive" pattern shared by RecipeManager's own
  delete and reparse buttons) and the existing `t.recipeManager.delete`/`confirmDelete` translation
  strings — the message ("Delete this recipe? This can't be undone.") reads exactly the same
  whether the button is reached from the backoffice or from a regular user's own Import page,
  because it's the literal same component and copy, not a lookalike. `RecipeList` (shared between
  the Import page's submissions grid and, structurally, wherever else it's reused) gained an
  optional `onDelete` prop following the same "presence of the callback controls whether the
  control renders" convention `onToggleShare` already used, rather than a separate boolean flag.
  <br><br>
  Reported live almost immediately after: importing a URL, deleting the resulting recipe, then
  trying to import the exact same URL again permanently 409'd with "This URL is already in the
  import queue" — false; nothing was queued or imported any more, the recipe was gone. Root cause:
  `POST /imports`'s duplicate-URL guard (`_ensure_not_duplicate`) checks for *any* `ImportJob` row
  matching the URL and creator, with no awareness of whether the recipe that job produced still
  exists — deleting the recipe was never taught to clean up the job record that led to it, so the
  guard kept tripping forever on a job whose only trace left in the system was itself. Regular
  users deleting their own imports made this newly reachable, but the same gap existed for an
  admin doing the same thing — this was latent before, not introduced by the delete feature. Fixed
  in `delete_recipe` itself: when the recipe being deleted has a `source_url`, every `ImportJob`
  row matching that exact `source` **and** the recipe's own `owner_user_id` (not necessarily the
  caller's id — an admin deleting someone else's import must free up *that owner's* URL, not their
  own) is deleted in the same transaction, before the recipe itself. Verified the test actually
  catches the regression, not just documents the fix: reverted the `recipes.py` change with the new
  test still in place and confirmed it fails with the exact same 409 the user hit, then restored
  the fix and confirmed it passes — and separately confirmed live via `curl` against the running
  dev API with a real JWT: 409 while the recipe exists, a real `204` delete, then a clean `201` on
  the identical URL resubmitted right after.
- **Importing a Google AMP-viewer link.** A user-reported URL
  (`google.com/amp/s/www.gustos.ro/amp/...`) failed to import. Not JS rendering, not a dead page —
  Google's own AMP-viewer, when the target no longer has a live AMP page, doesn't 3xx-redirect all
  the way to the real article; it lands on a Google-served "click through to continue" interstitial
  instead (`google.com/url?q=<real-url>&...`), a genuine `200 OK` with real HTML, so `httpx`'s
  `follow_redirects=True` has nothing left to follow — `fetch_page` was handing Claude that notice
  page instead of the recipe. Fixed by recognizing the landed-on notice page
  (`_resolve_google_redirect_notice`) and reading the real destination straight out of its own
  URL's `q=` param — deliberately *not* parsed out of the interstitial's HTML text, which is
  locale-dependent ("Notă de redirecționare" in Romanian, "Redirect Notice" in English, etc.); the
  URL shape is the one part of this that's stable across locales. `fetch_page` recurses into itself
  once resolved (capped at 3 hops, defensively — not a scenario expected in practice), which gets a
  fresh robots.txt + rate-limit check for the real target's domain for free, the same as any other
  fetch — and changed its return type from `str` to `(final_url, html)` so callers needing to
  resolve a relative URL (image `src` in particular) join against the page actually fetched, not
  whatever the requester originally pasted; `handlers.py` also now stores that resolved URL as the
  recipe's own `source_url`, so "View original recipe" points at the real article instead of
  back at Google's dead-end notice page.
  <br><br>
  A second, unrelated bug surfaced while fixing the first: the very first request (to
  `google.com/amp/s/...` itself) came back `disallowed`, tracing to a genuine limitation in
  Python's stdlib `urllib.robotparser` — confirmed live, it mis-parses Google's real robots.txt
  badly enough to disallow even a path Google's own file explicitly `Allow`s (`/search/about`),
  apparently from not correctly grouping multiple `User-agent:` lines that share one rule block
  (a real pattern in Google's file). Rather than swap robots.txt parsers for every site over one
  domain's quirk — a much larger, riskier change with no evidence any other scraped site is
  affected — this is sidestepped narrowly: an AMP-viewer or `/url` redirect link isn't a page to
  scrape in the first place, it's a redirect mechanism, the same as an ordinary HTTP 3xx, which
  this codebase already doesn't apply a fresh robots.txt check to per hop. `_is_google_redirect_url`
  recognizes that narrow shape (Google host **and** `/url` or `/amp/*` path) and skips the
  robots.txt permission check for just that one hop — still rate-limited, and the real target's own
  domain still gets a full, fresh robots.txt + rate-limit check once resolved, so nothing about
  real content-scraping ethics is bypassed, only the mechanical hop through Google's own redirect
  page. Verified against the real URL end to end, not a synthetic fixture: `docker compose exec
  worker python`, confirmed the resolved `source_url` pointed at the real, live `gustos.ro` article
  (not Google), a real photo downloaded (not a broken link), and both language translations landed
  correctly — then confirmed in the browser that the recipe rendered with a working "View original
  recipe" link.
- **Internal links must agree with the declared canonical, or Google overrides it.** Google Search
  Console flagged `https://easy2cook.app/ro` as "Duplicate, Google chose different canonical than
  user": the page's own `useSeoMeta` call already declared a correct, self-referencing canonical
  (`/ro`), so the mismatch wasn't a missing tag — it was the site's own internal link graph voting
  against it. This app has no SSR, so every route's raw `index.html` is byte-identical (no
  route-specific canonical exists until client JS runs); Google leans more heavily on other signals
  — chiefly internal links — when a site's raw HTML can't distinguish pages, and the header logo
  (`<Link to="/">`, rendered on literally every page) plus the "Recipes" nav link and several
  in-page CTAs (`<Link to="/recipes">`) were all pointing at the bare, unprefixed routes instead of
  the language-prefixed ones actually listed in `sitemap.xml` — the single largest internal-linking
  signal on the whole site was reinforcing exactly the URL we didn't want indexed. Fixed by pointing
  every one of those links at `/${language}` / `/${language}/recipes` instead (same target content,
  since `/` and `/{lang}` both render through `HomeRoute`), and by giving `RecipeBrowser` (rendered
  at both `/recipes` and `/{lang}/recipes`) the self-referencing canonical + hreflang alternates it
  was missing entirely — it had no `canonical` in its `useSeoMeta` call before this, so navigating
  there after visiting another page didn't just fail to help, it actively stripped any previously
  set canonical tag from the DOM. `LandingPage` also gained `hreflangAlternates` matching what
  `sitemap.xml` already declares for the home entries, so the language variants are unambiguously
  signaled as alternates rather than left for Google to infer from content similarity alone. The
  unprefixed routes themselves are deliberately kept working (see `RecipeDetail`'s own comment) —
  only which URL the site *points at itself* changed, not what still resolves.
- **Recipe search: `pg_trgm` over `tsvector`, title-only, AND-substring, diacritic-insensitive,
  client-side on the Import page.** Chose Postgres trigram similarity (`pg_trgm`, migration `0036`)
  over native full-text search (`tsvector`/`to_tsquery`) because `recipe_translations.language`
  varies row-to-row (`ro` or `en`) and `tsvector` needs a dictionary/config choice per row for
  stemming to work correctly — trigram substring matching (`ILIKE '%word%'` against a GIN index)
  needs none of that, and "type a few letters, see the list narrow" reads as a plain substring
  filter, not a ranked search engine, so prefix-only `tsquery` matching would actually have been a
  worse fit (it wouldn't match "choc" inside "Hot**choc**olate"). Scoped to title only, not
  description/ingredients — ingredients are a JSON array, not indexable the same simple way, and
  title covers the common case; extending scope later is an additive migration, not a rework.
  Multi-word queries AND every word as its own substring check (any order) rather than requiring an
  exact phrase, so "chicken soup" also matches "Soup with Chicken" — the more forgiving choice for
  a filter box, at the cost of being slightly less precise than an exact-phrase match. Both the
  title and the query are also normalized through `f_unaccent` (migration `0036`'s `IMMUTABLE`
  wrapper around the Postgres `unaccent` extension) before comparing, so a search is
  diacritic-insensitive in both directions — "briose" matches "Brioșe", and a query typed *with*
  diacritics still matches a title that has none — since real recipe titles in this app are
  inconsistent about diacritics depending on the source site, and requiring an exact accent match
  would silently miss recipes a user can plainly see the word in. `unaccent()` itself is only
  `STABLE`, not `IMMUTABLE` (it resolves its dictionary via `search_path` at call time), so it
  can't be used directly in an index expression — the wrapper pins it to the `unaccent` dictionary
  by name, making it deterministic and safe to index on; the same wrapper is called again at query
  time (`routers/recipes.py`'s `_unaccent_lower`) so both sides of the comparison are normalized
  identically. Postgres tests run against SQLite, which has no `unaccent()`, so `conftest.py`
  registers a Python equivalent (`unicodedata.normalize("NFKD", ...)`, stripping combining marks)
  as a custom SQLite function of the same name, keeping the fast unit-test suite exercising the
  same behavior without a real Postgres dependency. The Import page's "My recipes" list
  (`GET /users/me/submissions`) was already unpaginated — it loads everything up front — so its
  search filters client-side (`ui/searchMatch.ts`, the same AND-substring logic re-implemented in
  TypeScript, including its own diacritic fold via `String.prototype.normalize("NFKD")` + stripping
  the Unicode combining-marks block, no library needed) rather than adding a `search` param to an
  endpoint that has never taken any query params at all; the front office and back office both
  already go through the paginated `GET /recipes`, so they search server-side via the new `search`
  param there instead, composing with whatever other filter (category, Favorites) is already
  active exactly like those filters already compose with each other.
- **Recipe sharing: open/multi-use link, images physically duplicated, no forced copy-or-view
  choice, and a StrictMode bug the redirect flow surfaced.** `recipe_shares` links are open (anyone
  who has the token can use it) and multi-use (any number of different people can each
  independently view or copy it) rather than tied to one recipient's email or consumed on first
  use — simpler to build (no claim-locking, no email collection up front), and matches how link
  sharing already works everywhere else (Google Docs, Notion). The trade-off: a forwarded link
  works for whoever it's forwarded to, same as any other link-sharing model. Once authenticated,
  the recipient sees the full recipe *and* a "Copy to my recipes" button together, not a forced
  one-time fork between "view" or "copy" — a forced choice made before they've even seen what
  they're getting is the less friendly option, and nothing is lost by letting them decide anytime
  before the link expires. Copying **physically duplicates the recipe's image files**
  (`images.py`'s `copy_images`, new filenames) rather than pointing the copy at the original's same
  URLs — every other part of this codebase assumes one recipe owns its image files outright
  (`delete_recipe` unconditionally deletes every file in `recipe.images`), and a copy sharing the
  original's files would silently break the recipient's photos the moment the sender deleted their
  own recipe. Translations and `RecipeIngredientLink` rows are duplicated too, so the recipient's
  copy has full nutrition data immediately, no re-enrichment wait. The trickiest part wasn't the
  data model, it was getting a not-yet-authenticated visitor back to the exact share page after
  finishing auth: manual signup requires an email-verify hop through two other pages
  (signup → verify → login) before the visitor is actually logged in, so a query param can't
  survive the chain — a `cookbook-pending-share` localStorage key does instead (`sharing/
  pendingShare.ts`), read back once auth state flips true. The first version of that redirect lived
  in a separate `App()`-level `useEffect` and raced the `/login` route's own `<Navigate to="/">`;
  it was live-verified working, then broke on the very next live run, which is what exposed a real
  bug: under React's `<StrictMode>` (dev-only, `main.tsx`), effects run mount→cleanup→mount, and
  that effect wasn't idempotent — its second invocation re-read the pending token from localStorage
  *after* the first invocation had already cleared it, saw nothing, and silently redirected home,
  undoing the first (correct) navigation. Fixed by moving the decision into the route itself
  (`PostAuthRedirect`, App.tsx) and computing the redirect target exactly once via a lazy
  `useState` initializer (a pure read, safe to call twice) instead of re-deriving it inside the
  effect on every invocation — see `App.test.tsx`, which renders under `<StrictMode>` specifically
  to keep this class of bug caught. Sender's email is withheld from the pre-auth teaser (shown only
  once the visitor is authenticated) since the link is open — an unintended holder of a forwarded
  link shouldn't learn who sent it before they've even signed in.

  A follow-up request unified this with community sharing (`Recipe.is_shared` +
  `status=approved`) in the same dialog, rather than keeping them as two separately-triggered
  features: a recipe that's already public already has a permanent URL nobody needs an account to
  view, so generating a *second*, expiring, sign-up-walled link for it would be strictly worse —
  `ShareRecipeDialog` checks `recipe.status === "approved" && recipe.is_shared` (the frontend
  mirror of `_is_public`) and, when true, shows/copies that existing URL directly instead of
  calling `POST /recipes/{id}/shares` at all. No backend change was needed for that branch — it's
  pure UI, reusing the same URL `PublicRecipeDetail`/the sitemap already serve. This also settled
  where the entry point belongs: since a public recipe's "send" is just copying a URL (no
  privileged action), the icon shows unconditionally on every recipe card and detail page,
  including to a logged-out visitor viewing a public recipe — there's nothing to gate.

## License

Cookbook is licensed under the [GNU Affero General Public License v3.0](LICENSE) (AGPL-3.0).

Because this is self-hosted, network-facing software, AGPL was chosen over MIT/Apache-2.0/GPL
specifically to close the "SaaS loophole" plain GPL leaves open: under GPL, someone could take a
modified copy of Cookbook, run it as a paid hosted service, and never publish their changes, since
GPL's copyleft only triggers on *distributing* the software, not on running it as a network
service. AGPL adds that trigger — anyone who runs a modified version of Cookbook and lets others
use it over a network must also make that version's source available to those users.

Copyright (C) 2026 Dan Ionescu.
