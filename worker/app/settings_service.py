...
def get_settings(db: Session) -> SettingsSnapshot:
    ...
    Called once per import job (see handlers.handle_import_job), not per HTTP call within a
    job — one row read per job is cheap and means a setting change an admin applies takes
    effect on the very next job the worker picks up, with no cache to go stale and no restart.
    """
    row = db.get(AppSettings, SETTINGS_ROW_ID)
    if row is None:
        # Defensive fallback — mirrors api's migration 0006 seed defaults / app.config.Settings.
        # Shouldn't happen once the api container has applied that migration.
